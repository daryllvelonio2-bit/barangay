...
internal static class UxEnhancementsIntegration
{
    ...
    public static void GoBack()
    {
        ...
        if (route != null)
        {
            _mainWindow?.NavigatePage(route, recordHistory: false);
        }
    }
        ...
        try
        {
            switch (dialogId)
            {
                ...
                case "NewResident":
                    if (Permissions.Has(PermissionKeys.CreateResidents))
                    {
                        FullscreenDialogNavigator.Open(
                            new ResidentDetailsWindow(),
                            "Add New Resident",
                            "Enter the resident's identity, address, and household information.",
                            "ResidentWorkspace",
                            IconChar.UserPlus,
                            "Save Resident");
                    }
                ...
                case "NewCertificate":
                    ...
                    {
                        FullscreenDialogNavigator.Open(
                            new CertificationWindow(),
                            "New Certificate Request",
                            "Select a resident, document type, purpose, and payment details.",
                            "Clearances",
    ...
    private static void OnRefreshRequested()
    {
        if (_navHistory?.Current != null)
        {
            ...
            _mainWindow?.NavigatePage(currentRoute, recordHistory: false, forceReload: true);
