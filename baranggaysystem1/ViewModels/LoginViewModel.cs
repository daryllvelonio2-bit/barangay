...
public sealed partial class LoginViewModel : ViewModelBase
{
	...
	private static bool PerformLogin(string username, string password)
	{
		if (OfflineDatabaseSupport.IsOffline || DBConnection.IsSqliteSelected())
		{
			...
			if (!OfflineDatabaseSupport.TryAuthenticateOffline(username, password, out int offlineUserId, out int offlineBarangayId, out string offlineRole))
			{
				throw new UnauthorizedAccessException("Invalid username or password.");
			}
			SetSession(offlineUserId, offlineBarangayId, offlineRole, username);
			return IsAdminRole(offlineRole);
		}
		try
		{
		...
		{
			AppLogger.LogError("MySQL login failed.", ex);
			throw new UnauthorizedAccessException("Could not connect to database. Check your connection settings.");
		}
	}
