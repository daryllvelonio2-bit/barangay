...
public partial class HouseholdDetailsViewModel : ObservableObject
{
	...
	private async Task SetAsHead(HouseholdMember? member)
	{
		...
		{
			DialogService.Instance.ShowWarning("You do not have permission to change the head of family.");
			return;
		}
		try
		{
			IsLoading = true;
			await EnsureResidentHouseholdSchemaAsync();
			await using DatabaseTransactionScope transaction = await DatabaseTransactionScope.BeginAsync();
			await transaction.ExecuteNonQueryAsync(
				"UPDATE resident SET is_head_of_family = 0 WHERE household_id = @householdId",
				new Dictionary<string, object?> { ["@householdId"] = _householdId.Value });
			int affected = await transaction.ExecuteNonQueryAsync(
				@"UPDATE resident SET is_head_of_family = 1
			...
			if (affected <= 0)
			{
				throw new InvalidOperationException("The selected resident is no longer a member of this household.");
			}
			await transaction.CommitAsync();
			AuditTrailService.Log("Households", "resident", member.ResidentId, "SET_FAMILY_HEAD",
				null, new { HouseholdId = _householdId.Value, member.ResidentId },
