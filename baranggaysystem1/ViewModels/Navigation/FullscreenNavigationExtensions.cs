...
{
    /// <summary>
    /// Invokes the active fullscreen view's save callback exactly once.
    /// Forms that save without immediately navigating back use this to refresh
    /// their origin page without retaining a callback that could run twice.
    /// </summary>
    internal static void InvokeOnSavedCallback()
    {
        if (NavigationService.Instance.CurrentView is not FullscreenViewHost host)
        {
            return;
        }

        Action? callback = host.SavedCallback;
        host.SavedCallback = null;
        callback?.Invoke();
    }
    ...
    public static void NavigateToFullscreen(this NavigationService nav, FullscreenViewConfig config)
    {
        ...
        if (config.Content is DialogContentAdapter adapter)
        {
            adapter.DialogCompleted += (sender, success) =>
            {
                nav.NavigateBackFromFullscreen(config.OriginRoute, refreshOnReturn: success);
            };
        }
        ...
        if (config.ToolbarItems != null)
        {
            foreach (var item in config.ToolbarItems)
            {
                if (item is System.Windows.Controls.Button button &&
                    button.Tag is FullscreenToolbarActionMetadata action)
                {
                    button.Tag = null;
                    if (config.Content is DialogContentAdapter)
                    {
                        // Window-based forms already contain their primary and cancel
                        // actions. Do not duplicate those actions in the host header.
                        continue;
                    }
                    else
                    {
                        button.Click += action.FallbackHandler;
                    }
                }
        ...
        host.SavedCallback = config.OnSaved;
        ...
        bool refreshOnReturn = false)
    {
        ...
        Action? onSaved = (nav.CurrentView as FullscreenViewHost)?.SavedCallback;
