...
/// </summary>
public abstract class FullscreenFormBase : UserControl, IUnsavedChangesSource
{
    ...
    public async Task<bool> TrySaveAsync()
    {
        ...
        // Re-validate rather than blocking blindly so the user sees the current error state.
        if (HasValidationErrors && !ValidateAndDisplayErrors())
        {
            return false;
        }
        ...
        try
        {
            ...
            {
                // Requirement 6.4: Exception — log, show in-form, show error toast, preserve input
                helper.AppLogger.LogError("Fullscreen form save failed.", ex);
                ValidationPanel?.ShowError(ex.Message);
