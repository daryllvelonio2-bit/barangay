...
public partial class AyudaProgramViewModel : ObservableObject
{
	...
	public async Task InitializeAsync()
	{
		...
		try
		{
			...
			var loadTask = _ayudaService.GetProgramAsync(_programId.Value);
			...
			if (completedTask == timeoutTask || loadTask.IsFaulted)
			{
				AppLogger.LogWarning("Ayuda program editor timed out or failed to load.");
				DialogService.Instance.ShowWarning("Could not load the ayuda program. The database may be unavailable.");
				this.CloseRequested?.Invoke(false);
				return;
			}

			AyudaProgramRecord ayudaProgramRecord = loadTask.Result;
	...
	private async Task SaveProgram()
	{
		...
		{
			IsProcessing = true;
			var saveTask = _ayudaService.SaveProgramAsync(new AyudaProgramRecord
			{
			...
			{
				DialogService.Instance.ShowError("Save operation timed out. The database may be unavailable.", "Ayuda Program");
				return;
			}

			// Propagate any exception from the save task
