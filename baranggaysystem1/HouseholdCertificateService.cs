...
internal static class HouseholdCertificateService
{
	...
	private static List<CertificateMemberRow> LoadMembersInternal(int householdId, int barangayId, bool includeHeadOfFamily)
	{
		...
		DataTable table = DbHelper.LoadTable(
			...
			"       CASE WHEN birth_date IS NULL THEN NULL ELSE TIMESTAMPDIFF(YEAR, birth_date, CURDATE()) END AS age,\n" +
			"       COALESCE(sex, '') AS sex,\n" +
			...
			"ORDER BY " + text2 + "last_name, first_name, middle_name",
			command =>
		{
		...
		{
			string value = Convert.ToString(row["sex"]) ?? string.Empty;
			list.Add(new CertificateMemberRow
			{
				FullName = Convert.ToString(row["full_name"]) ?? "Household Member",
				AgeLabel = row["age"] == DBNull.Value
					? "-"
					: Convert.ToString(row["age"], CultureInfo.InvariantCulture) ?? "-",
				SexLabel = NormalizeSex(value),
				StatusLabel = Convert.ToString(row["status"]) ?? "ACTIVE",
				IsHead = Convert.ToInt32(row["is_head_of_family"], CultureInfo.InvariantCulture) == 1
			});
		}
		return list;
	}
	...
	private static void TryEnsureResidentSchemaCompatibility()
	{
		try
		{
			...
			{
				OfflineDatabaseSupport.EnsureInitialised();
			}
			else
			{
				...
				@"SELECT CONCAT_WS(' ', r.first_name, r.middle_name, r.last_name) AS full_name,
				  ...
				  ORDER BY
				    CASE
				      ...
				      ELSE 9
				    END,
				    bo.position, r.last_name, r.first_name
				  LIMIT 1",
				command => command.Parameters.AddWithValue("@barangayId", barangayId));
			if (table.Rows.Count > 0)
			{
				DataRow row = table.Rows[0];
				string name = Convert.ToString(row["full_name"]) ?? string.Empty;
				string position = Convert.ToString(row["position"]) ?? string.Empty;
				if (!string.IsNullOrWhiteSpace(name))
				{
					return new SignatoryInfo
					{
						Name = name,
						Position = string.IsNullOrWhiteSpace(position) ? "Barangay Official" : position
					};
				}
			}
		}
