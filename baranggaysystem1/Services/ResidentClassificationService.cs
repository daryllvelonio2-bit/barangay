...
internal sealed class ResidentClassificationService
{
	...
	{
		AuthorizationGuard.RequireAdmin("manage resident tags and categories");
		await EnsureSchemaReadyAsync(cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
		...
		if (sanitized.ClassificationId > 0)
		{
			...
			AuditTrailService.Log("Residents", "resident_classification", sanitized.ClassificationId,
				"UPDATE", record, sanitized, "Resident classification updated.");
		...
		AuditTrailService.Log("Residents", "resident_classification", newId,
			"CREATE", null, sanitized, "Resident classification created.");
		return newId;
	}
	...
	{
		AuthorizationGuard.RequireAdmin("change resident tag and category status");
	...
	{
		AuthorizationGuard.RequireAdmin("archive resident tags and categories");
		await SetStatusAsync(classificationId, "ARCHIVED", cancellationToken).ConfigureAwait(false);
	}
