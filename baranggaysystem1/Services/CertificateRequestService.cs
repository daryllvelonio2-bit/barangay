...
internal sealed class CertificateRequestService
{
	...
	public async Task<IReadOnlyList<CertificateDocumentTypeOption>> GetDocumentTypesAsync()
	{
		...
		DataTable obj = await DatabaseManagerAsync.LoadTableAsync("\n                SELECT document_type_id AS doc_type_id,\n                       name,\n                       COALESCE(code, '') AS code,\n                       0.00 AS fee_default,\n                       validity_days\n                FROM document_type\n                ORDER BY name ASC, document_type_id ASC").ConfigureAwait(continueOnCapturedContext: false);
	...
	public async Task<CertificateRequestDraft?> GetRequestAsync(int requestId)
	{
		...
		return new CertificateRequestDraft
		{
			...
			DocTypeId = Convert.ToInt32(dataRow["document_type_id"], CultureInfo.InvariantCulture),
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.RequestCertificates, "create certificate requests");
		await DatabaseManagerAsync.ExecuteNonQueryAsync("\n                INSERT INTO document_request\n                    (barangay_id, doc_type_id, document_type_id, resident_id, purpose, status, requested_at, requested_by_user_id, remarks, fee, or_number, business_name, business_nature)\n                VALUES\n                    (@barangayId, @docTypeId, @docTypeId, @residentId, @purpose, 'SUBMITTED', NOW(), @userId, @remarks, @fee, @orNumber, @businessName, @businessNature)", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@barangayId", (object)UserSession.BarangayId);
		...
		AuditTrailService.Log("Certificates", "document_request", draft.ResidentId, "SUBMIT", null,
			new { draft.ResidentId, draft.DocTypeId, draft.Purpose, draft.Fee },
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.IssueCertificates, "release certificate requests");
		if (!draft.RequestId.HasValue || draft.RequestId.Value <= 0)
		{
			throw new InvalidOperationException("Submit and approve the request before releasing the document.");
		}
		CertificateRequestDraft current = await GetRequestAsync(draft.RequestId.Value).ConfigureAwait(false)
			?? throw new InvalidOperationException("The selected request no longer exists.");
		if (!string.Equals(current.Status, "APPROVED", StringComparison.OrdinalIgnoreCase) &&
			!string.Equals(current.Status, "READY_FOR_RELEASE", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidOperationException("Only approved requests can be released.");
		}
		if (draft.Fee > 0m && string.IsNullOrWhiteSpace(draft.OrNumber))
		{
			throw new InvalidOperationException("Record the official receipt number before releasing a paid document.");
		}
		...
		if (affected != 1)
		{
			throw new DBConcurrencyException("The request changed before it could be released. Reload the queue and try again.");
		}
		AuditTrailService.Log("Certificates", "document_request", draft.RequestId.Value, "RELEASE",
			new { current.Status }, new { Status = "RELEASED", DocumentNo = documentNo },
			"Approved document released and printed.");
	...
	public async Task ApproveRequestAsync(int requestId, string documentLabel)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ApproveCertificates, "approve certificate requests");
		int affected = await DatabaseManagerAsync.ExecuteNonQueryAsync(
			"UPDATE document_request " +
			"SET status = 'APPROVED', approved_at = CURRENT_TIMESTAMP, approved_by_user_id = @userId, remarks = @remarks " +
			"WHERE doc_request_id = @requestId AND UPPER(COALESCE(status, 'SUBMITTED')) = 'UNDER_REVIEW'",
			cmd =>
			{
				cmd.Parameters.AddWithValue("@userId", NormalizeNullableUserId(UserSession.UserId));
				cmd.Parameters.AddWithValue("@remarks", documentLabel + " was reviewed and approved.");
				cmd.Parameters.AddWithValue("@requestId", requestId);
			}).ConfigureAwait(false);
		if (affected != 1)
		{
			throw new DBConcurrencyException("Only requests under review can be approved. Start the review first, then reload the queue.");
		}
		AuditTrailService.Log("Certificates", "document_request", requestId, "APPROVE",
			new { Status = "UNDER_REVIEW" }, new { Status = "APPROVED" },
			documentLabel + " approved.");
	}

	public async Task StartReviewAsync(int requestId, string documentLabel)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ApproveCertificates, "review certificate requests");
		int affected = await DatabaseManagerAsync.ExecuteNonQueryAsync(
			"UPDATE document_request " +
			"SET status = 'UNDER_REVIEW', remarks = @remarks " +
			"WHERE doc_request_id = @requestId AND UPPER(COALESCE(status, 'SUBMITTED')) = 'SUBMITTED'",
			cmd =>
			{
				cmd.Parameters.AddWithValue("@remarks", documentLabel + " is under staff review.");
				cmd.Parameters.AddWithValue("@requestId", requestId);
			}).ConfigureAwait(false);
		if (affected != 1)
		{
			throw new DBConcurrencyException("Only submitted requests can enter review. Reload the queue.");
		}
		AuditTrailService.Log("Certificates", "document_request", requestId, "START_REVIEW",
			new { Status = "SUBMITTED" }, new { Status = "UNDER_REVIEW" },
			documentLabel + " placed under review.");
	}

	public async Task CancelRequestAsync(int requestId, string documentLabel, string reason)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.CancelCertificates, "cancel certificate requests");
		string cleanReason = (reason ?? string.Empty).Trim();
		if (cleanReason.Length < 5)
		{
			throw new InvalidOperationException("Provide a cancellation reason with at least five characters.");
		}
		int affected = await DatabaseManagerAsync.ExecuteNonQueryAsync("\n                UPDATE document_request\n                SET status = 'CANCELLED',\n                    remarks = @remarks\n                WHERE doc_request_id = @requestId\n                  AND UPPER(COALESCE(status, 'SUBMITTED')) NOT IN ('RELEASED', 'CANCELLED')", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@remarks", (object)(documentLabel + " cancelled. Reason: " + cleanReason));
			cmd.Parameters.AddWithValue("@requestId", (object)requestId);
		}).ConfigureAwait(continueOnCapturedContext: false);
		if (affected != 1)
		{
			throw new DBConcurrencyException("The request changed before it could be cancelled. Reload the queue.");
		}
		AuditTrailService.Log("Certificates", "document_request", requestId, "CANCEL", null,
			new { Status = "CANCELLED", Reason = cleanReason },
			documentLabel + " cancelled: " + cleanReason);
	}
