...
internal sealed class TanodService
{
    ...
    public async Task<DataTable> GetMemberAsync(int tanodId)
    {
        return await DatabaseManagerAsync.LoadTableAsync(
            "SELECT * FROM tanod_member WHERE tanod_id = @id LIMIT 1",
            cmd => cmd.Parameters.AddWithValue("@id", (object)tanodId)).ConfigureAwait(false);
    }
    ...
    {
        AuthorizationGuard.RequireAdmin("register tanod members");
    ...
    {
        AuthorizationGuard.RequireAdmin("update tanod members");
    ...
    public async Task<int> CreateShiftAsync(DateTime shiftDate, string shiftType,
        ...
        int[]? tanodIds = null)
    {
        AuthorizationGuard.RequireAdmin("schedule tanod shifts");
    ...
    {
        AuthorizationGuard.RequireAdmin("record tanod attendance");
    ...
    {
        AuthorizationGuard.RequireAdmin("create tanod patrol logs");
