...
public static class PasswordResetService
{
    ...
    public static void EnsureSchema()
    {
        try
        {
            if (OfflineDatabaseSupport.IsOffline ||
                ...
                DbConnectionSettingsStore.IsSqliteSelected())
            {
                if (!OfflineDatabaseSupport.EnsureInitialised())
                {
                    throw new InvalidOperationException("The local account recovery database is unavailable.");
                }
                return;
            }
    ...
    public static PasswordResetResult AdminReset(int targetUserId, int adminUserId)
    {
        ...
        try
        {
            ...
            if (UseLocalDatabase())
            {
                return AdminResetLocal(targetUserId, adminUserId);
            }
        ...
        string username, string answer1, string? answer2 = null)
    {
        ...
        try
        {
            ...
            if (UseLocalDatabase())
            {
                return ResetViaSecurityQuestionsLocal(username, answer1, answer2);
            }
    ...
    public static SecurityQuestionSet? GetSecurityQuestions(string username)
    {
        ...
        try
        {
            ...
            if (UseLocalDatabase())
            {
                int localUserId = DbHelper.ExecuteScalar<int>(
                    "SELECT user_id FROM user_account WHERE username = @u AND is_active = 1 LIMIT 1",
                    command => command.Parameters.AddWithValue("@u", username.Trim()));
                if (localUserId <= 0)
                {
                    return null;
                }
                var table = DbHelper.LoadTable(
                    ...
                    command => command.Parameters.AddWithValue("@uid", localUserId));
                ...
                {
                    Question1 = Convert.ToString(table.Rows[0]["question_1"]) ?? string.Empty,
                    Question2 = Convert.ToString(table.Rows[0]["question_2"]) ?? string.Empty
                };
            }
            MySqlConnection connection = DBConnection.GetConnection();
            try
    ...
    private static PasswordResetResult AdminResetLocal(int targetUserId, int adminUserId)
    {
        string tempPassword = GenerateTemporaryPassword();
        string hashedPassword = PasswordHelper.HashPassword(tempPassword);
        int affected = DbHelper.ExecuteNonQuery(
            @"UPDATE user_account
              SET password_hash = @hash, must_change_password = 1, updated_at = CURRENT_TIMESTAMP
              WHERE user_id = @userId AND is_active = 1",
            command =>
            {
        ...
        DbHelper.ExecuteNonQuery(
            @"INSERT INTO password_reset
                (user_id, reset_token, expires_at, is_used, used_at, initiated_by, reset_method)
              VALUES
                (@userId, @token, @expires, 1, CURRENT_TIMESTAMP, @adminId, 'ADMIN')",
            command =>
            {
                command.Parameters.AddWithValue("@userId", targetUserId);
                command.Parameters.AddWithValue("@token", GenerateResetToken());
                command.Parameters.AddWithValue("@expires", DateTime.UtcNow.AddMinutes(TokenExpiryMinutes));
    ...
    private static PasswordResetResult ResetViaSecurityQuestionsLocal(
        string username,
        string answer1,
        string? answer2)
    {
        ...
        var answers = DbHelper.LoadTable(
        ...
        {
            return PasswordResetResult.Failure("No security questions are configured for this account.");
        }
        string hash1 = Convert.ToString(answers.Rows[0]["answer_1_hash"]) ?? string.Empty;
        string hash2 = Convert.ToString(answers.Rows[0]["answer_2_hash"]) ?? string.Empty;
        if (PasswordHelper.VerifyPassword(answer1.Trim().ToLowerInvariant(), hash1, out _) ==
            PasswordHelper.VerificationResult.Failed)
        {
            return PasswordResetResult.Failure("Security answer(s) are incorrect.");
        }
        if (!string.IsNullOrWhiteSpace(hash2))
        {
            if (string.IsNullOrWhiteSpace(answer2) ||
                PasswordHelper.VerifyPassword(answer2.Trim().ToLowerInvariant(), hash2, out _) ==
                PasswordHelper.VerificationResult.Failed)
            {
                return PasswordResetResult.Failure("Security answer(s) are incorrect.");
            }
        ...
        DbHelper.ExecuteNonQuery(
            @"INSERT INTO password_reset
                (user_id, reset_token, expires_at, is_used, used_at, reset_method)
              VALUES
                (@userId, @token, @expires, 1, CURRENT_TIMESTAMP, 'SECURITY_QUESTION')",
            command =>
            {
                command.Parameters.AddWithValue("@userId", userId);
                command.Parameters.AddWithValue("@token", GenerateResetToken());
    ...
    private static bool VerifySecurityAnswers(MySqlConnection conn, int userId, string answer1, string? answer2)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(hash2))
            {
                if (string.IsNullOrWhiteSpace(answer2)) return false;
