...
internal sealed class FacilityBookingService
{
    ...
    {
        AuthorizationGuard.RequireAdmin("create facilities");
    ...
    {
        AuthorizationGuard.RequireAdmin("update facilities");
    ...
    {
        AuthorizationGuard.RequireAdmin("create facility bookings");
        if (startAt >= endAt)
    ...
    {
        AuthorizationGuard.RequireAdmin("update facility bookings");
        DataTable existing = await GetBookingAsync(bookingId).ConfigureAwait(false);
        if (existing.Rows.Count == 0)
            throw new InvalidOperationException("The selected booking no longer exists.");
        string currentStatus = existing.Rows[0]["status"]?.ToString() ?? string.Empty;
        if (IsTerminalStatus(currentStatus))
            throw new InvalidOperationException("Completed, rejected, and cancelled bookings are locked.");
    ...
    {
        AuthorizationGuard.RequireAdmin("change facility booking status");
        ...
        string current = row["status"]?.ToString()?.ToUpperInvariant() ?? string.Empty;
        if (current == normalized)
            throw new InvalidOperationException($"This booking is already {normalized.ToLowerInvariant()}.");
        if (!IsAllowedTransition(current, normalized))
            throw new InvalidOperationException(
                $"A {current.ToLowerInvariant()} booking cannot be changed to {normalized.ToLowerInvariant()}.");

        string? cleanReason = string.IsNullOrWhiteSpace(reason) ? null : reason.Trim();
        if ((normalized == "REJECTED" || normalized == "CANCELLED")
            && (cleanReason == null || cleanReason.Length < 5))
            throw new InvalidOperationException("Provide a reason with at least 5 characters.");

        if (normalized == "APPROVED")
        {
            int facilityId = Convert.ToInt32(row["facility_id"]);
            DateTime startAt = Convert.ToDateTime(row["start_at"]);
            DateTime endAt = Convert.ToDateTime(row["end_at"]);
            if (await HasConflictAsync(facilityId, startAt, endAt, bookingId).ConfigureAwait(false))
                throw new InvalidOperationException(
                    "This booking now conflicts with another pending or approved reservation.");
        }
        ...
        await DatabaseManagerAsync.ExecuteNonQueryAsync(sql, cmd =>
        {
            ...
            cmd.Parameters.AddWithValue("@reason", DbNullIfEmpty(cleanReason));
        }).ConfigureAwait(false);
        AuditTrailService.Log("FacilityBooking", "facility_booking", bookingId, "STATUS_CHANGE",
            new { status = current }, new { status = normalized, reason = cleanReason },
            $"Booking changed from {current} to {normalized}.");
    }

    public async Task UpdatePaymentStatusAsync(int bookingId, string paymentStatus)
    {
        AuthorizationGuard.RequireAdmin("change facility booking payment status");
    ...
    private static bool IsTerminalStatus(string? status)
    {
        ...
        return value == "REJECTED" || value == "COMPLETED" || value == "CANCELLED";
    }
    ...
    private static bool IsAllowedTransition(string current, string next)
    {
        ...
        {
            "PENDING" => next == "APPROVED" || next == "REJECTED" || next == "CANCELLED",
            "APPROVED" => next == "COMPLETED" || next == "CANCELLED",
            _ => false
        };
    }
}
