...
internal sealed class AnnouncementService
{
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageAnnouncements, "create announcements");
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageAnnouncements, "update announcements");
		...
		if (string.Equals(before.Status, "Archived", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidOperationException("Archived announcements are locked and cannot be edited.");
		}
		AnnouncementRecord sanitized = Sanitize(record);
	...
	{
		await ArchiveAnnouncementAsync(announcementId, "Archived from the announcements workflow.").ConfigureAwait(false);
	}

	public async Task ArchiveAnnouncementAsync(int announcementId, string reason)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageAnnouncements, "archive announcements");
		...
		if (string.Equals(before.Status, "Archived", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidOperationException("This announcement is already archived.");
		}
		await DatabaseManagerAsync.ExecuteNonQueryAsync("UPDATE announcements SET status = 'Archived', is_pinned = 0 WHERE announcement_id = @announcementId", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@announcementId", (object)announcementId);
		}).ConfigureAwait(continueOnCapturedContext: false);
		AuditTrailService.Log("Announcements", "announcement", announcementId, "ARCHIVE", before,
			new { status = "Archived", isPinned = false, reason = cleanReason },
			"Announcement archived: " + cleanReason);
	}
	...
	private static string RequireReason(string? reason)
	{
		...
		if (value.Length < 5)
		{
			throw new InvalidOperationException("Provide an archive reason with at least 5 characters.");
		}
		return value;
	}
