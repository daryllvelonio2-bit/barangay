...
public class StaffService
{
	...
	{
		AuthorizationGuard.RequireSuperAdmin("create staff accounts");
		...
		AuditTrailService.Log("Users", "user_account", userId, "CREATE", null,
			new { staff.Username, Role = roleName, staff.IsActive },
			"Staff account and role created atomically.");
		InvalidateLookupCaches();
	}

	public async Task UpdateStaffAsync(Staff staff)
	{
		AuthorizationGuard.RequireSuperAdmin("update staff accounts");
		Dictionary<string, object> parameters = new Dictionary<string, object>
		{
		...
		int? roleId = string.IsNullOrWhiteSpace(staff.Role) ? null : await ResolveRoleIdAsync(staff.Role);
		await using (DatabaseTransactionScope transaction = await DatabaseTransactionScope.BeginAsync().ConfigureAwait(false))
		{
			int affected = await transaction.ExecuteNonQueryAsync("\n                UPDATE user_account\n                SET first_name = @firstName,\n                    middle_name = @middleName,\n                    last_name = @lastName,\n                    full_name = @fullName,\n                    email = @email,\n                    contact_no = @contactNo,\n                    position = @position,\n                    department = @department,\n                    is_active = @isActive,\n                    updated_at = CURRENT_TIMESTAMP\n                WHERE user_id = @userId", parameters);
			if (affected != 1)
			{
				throw new DBConcurrencyException("The staff account changed or was removed. Reload the list.");
			}
			if (roleId.HasValue)
			{
				int roleUpdated = await transaction.ExecuteNonQueryAsync(
		...
		AuditTrailService.Log("Users", "user_account", staff.StaffId, "UPDATE", null,
			new { staff.Username, staff.Role, staff.IsActive },
			"Staff account and role updated atomically.");
		InvalidateLookupCaches();
	}

	public async Task SetStaffActiveStatusAsync(int staffId, bool isActive, string reason)
	{
		AuthorizationGuard.RequireSuperAdmin(isActive ? "reactivate staff accounts" : "deactivate staff accounts");
		string cleanReason = (reason ?? string.Empty).Trim();
		if (!isActive && cleanReason.Length < 5)
		{
			throw new InvalidOperationException("Provide a deactivation reason with at least five characters.");
		}
		if (staffId == UserSession.UserId && !isActive)
		{
			throw new InvalidOperationException("You cannot deactivate your own signed-in account.");
		}
		int affected = await DatabaseManagerAsync.ExecuteNonQueryAsync("UPDATE user_account SET is_active = @isActive, updated_at = CURRENT_TIMESTAMP WHERE user_id = @userId", delegate(MySqlCommand cmd)
		{
		...
		AuditTrailService.Log("Users", "user_account", staffId, isActive ? "REACTIVATE" : "DEACTIVATE",
			null, new { IsActive = isActive, Reason = cleanReason },
			isActive ? "Staff account reactivated." : "Staff account deactivated: " + cleanReason);
	}

	public async Task DeleteStaffAsync(int staffId, string reason)
	{
		await SetStaffActiveStatusAsync(staffId, isActive: false, reason);
	}
