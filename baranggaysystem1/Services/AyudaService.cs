...
internal sealed class AyudaService
{
	...
	public async Task<IReadOnlyList<AyudaProgramOption>> GetProgramOptionsAsync(int? includeProgramId = null)
	{
		...
		return (from row in (await DatabaseManagerAsync.LoadTableAsync("\n                SELECT ap.program_id,\n                       ap.program_name,\n                       ap.category,\n                       IFNULL(ap.allocated_budget, 0.00) AS allocated_budget,\n                       IFNULL(SUM(CASE WHEN UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED' THEN IFNULL(ar.amount, 0.00) ELSE 0.00 END), 0.00) AS spent_budget,\n                       CASE\n                           WHEN IFNULL(ap.allocated_budget, 0.00) - IFNULL(SUM(CASE WHEN UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED' THEN IFNULL(ar.amount, 0.00) ELSE 0.00 END), 0.00) < 0\n                               THEN 0.00\n                           ELSE IFNULL(ap.allocated_budget, 0.00) - IFNULL(SUM(CASE WHEN UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED' THEN IFNULL(ar.amount, 0.00) ELSE 0.00 END), 0.00)\n                       END AS remaining_budget,\n                       UPPER(COALESCE(ap.status, 'ACTIVE')) AS status,\n                       ap.start_date,\n                       ap.end_date\n                FROM ayuda_program ap\n                LEFT JOIN ayuda_release ar ON ar.program_id = ap.program_id\n                WHERE ap.barangay_id = @barangayId\n                GROUP BY ap.program_id, ap.program_name, ap.category, ap.allocated_budget, ap.status, ap.start_date, ap.end_date\n                HAVING (\n                    UPPER(COALESCE(ap.status, 'ACTIVE')) = 'ACTIVE'\n                    AND remaining_budget > 0\n                )\n                   OR (@includeProgramId > 0 AND ap.program_id = @includeProgramId)\n                ORDER BY ap.program_name ASC", delegate(MySqlCommand cmd)
			...
			select new AyudaProgramOption
			{
				...
				AllocatedBudget = ((row["allocated_budget"] == DBNull.Value) ? 0m : Convert.ToDecimal(row["allocated_budget"], CultureInfo.InvariantCulture)),
				SpentBudget = ((row["spent_budget"] == DBNull.Value) ? 0m : Convert.ToDecimal(row["spent_budget"], CultureInfo.InvariantCulture)),
	...
	public async Task<AyudaResidentAssistanceSummary> GetResidentAssistanceSummaryAsync(int residentId, int programId)
	{
		if (residentId <= 0)
		{
			return new AyudaResidentAssistanceSummary();
		}
		await EnsureSchemaReadyAsync().ConfigureAwait(continueOnCapturedContext: false);
		DataTable dataTable = await DatabaseManagerAsync.LoadTableAsync("\n                SELECT COUNT(ar.release_id) AS release_count,\n                       IFNULL(SUM(ar.amount), 0.00) AS total_amount,\n                       MAX(ar.released_at) AS last_release_at,\n                       SUM(CASE WHEN ar.program_id = @programId THEN 1 ELSE 0 END) AS program_release_count,\n                       IFNULL(SUM(CASE WHEN ar.program_id = @programId THEN ar.amount ELSE 0.00 END), 0.00) AS program_total_amount,\n                       MAX(CASE WHEN ar.program_id = @programId THEN ar.released_at ELSE NULL END) AS last_program_release_at\n                FROM ayuda_release ar\n                INNER JOIN ayuda_program ap ON ap.program_id = ar.program_id\n                WHERE ar.resident_id = @residentId\n                  AND ap.barangay_id = @barangayId\n                  AND UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED'", delegate(MySqlCommand cmd)
		{
		...
		int householdReleaseCount = await DatabaseManagerAsync.ExecuteScalarAsync<int>("\n                SELECT COUNT(ar.release_id)\n                FROM resident selected_resident\n                INNER JOIN resident household_member\n                    ON household_member.household_id = selected_resident.household_id\n                   AND household_member.resident_id <> selected_resident.resident_id\n                INNER JOIN ayuda_release ar ON ar.resident_id = household_member.resident_id\n                INNER JOIN ayuda_program ap ON ap.program_id = ar.program_id\n                WHERE selected_resident.resident_id = @residentId\n                  AND selected_resident.household_id IS NOT NULL\n                  AND ap.barangay_id = @barangayId\n                  AND ar.released_at >= @cutoffDate\n                  AND UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED'", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@residentId", (object)residentId);
			cmd.Parameters.AddWithValue("@barangayId", (object)ResolveBarangayId());
			cmd.Parameters.AddWithValue("@cutoffDate", (object)DateTime.Today.AddDays(-90));
		}).ConfigureAwait(continueOnCapturedContext: false);
		return new AyudaResidentAssistanceSummary
		{
			ReleaseCount = ReadInt(row, "release_count"),
			TotalAmount = ReadDecimal(row, "total_amount"),
			LastReleaseAt = ReadDateTime(row, "last_release_at"),
			ProgramReleaseCount = ReadInt(row, "program_release_count"),
			ProgramTotalAmount = ReadDecimal(row, "program_total_amount"),
			LastProgramReleaseAt = ReadDateTime(row, "last_program_release_at"),
			RecentHouseholdReleaseCount = householdReleaseCount
		};
	}
	...
	{
		AuthorizationGuard.RequireAdmin("manage ayuda budget programs");
		await EnsureSchemaReadyAsync().ConfigureAwait(continueOnCapturedContext: false);
	...
	{
		throw new InvalidOperationException("Permanent deletion is disabled. Archive the ayuda program instead.");
	}

	public async Task ArchiveProgramAsync(int programId, string reason)
	{
		AuthorizationGuard.RequireAdmin("archive ayuda budget programs");
		...
		await DatabaseManagerAsync.ExecuteNonQueryAsync("UPDATE ayuda_program\n                  SET status = 'ARCHIVED', notes = @notes, updated_by_user_id = @userId, updated_at = CURRENT_TIMESTAMP\n                  WHERE program_id = @programId\n                    AND barangay_id = @barangayId", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@programId", (object)programId);
			cmd.Parameters.AddWithValue("@barangayId", (object)ResolveBarangayId());
			cmd.Parameters.AddWithValue("@notes", (object)(before.Notes + "\nArchived: " + cleanReason).Trim());
			cmd.Parameters.AddWithValue("@userId", GetUserIdOrNull());
		}).ConfigureAwait(continueOnCapturedContext: false);
		AuditTrailService.Log("Ayuda", "ayuda_program", programId, "ARCHIVE", before,
			new { Status = "ARCHIVED", Reason = cleanReason },
			"Ayuda budget program archived: " + cleanReason);
	}
	...
	{
		AuthorizationGuard.RequireAdmin("post ayuda distributions");
		await EnsureSchemaReadyAsync().ConfigureAwait(continueOnCapturedContext: false);
	...
	public async Task UpdateReleaseAsync(AyudaReleaseRecord record)
	{
		...
		throw new InvalidOperationException(
			"Posted ayuda releases are permanent. Cancel the incorrect release with a reason, then post a corrected distribution.");
	}

	public async Task CancelReleaseAsync(int releaseId, string reason)
	{
		AuthorizationGuard.RequireAdmin("reverse ayuda distributions");
		string cleanReason = (reason ?? string.Empty).Trim();
		if (cleanReason.Length < 5)
		{
			throw new InvalidOperationException("Provide a reversal reason with at least five characters.");
		}
		await EnsureSchemaReadyAsync().ConfigureAwait(continueOnCapturedContext: false);
		...
		if (string.Equals(before.ReleaseStatus, "CANCELLED", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidOperationException("This ayuda release is already cancelled.");
		}
		await DatabaseManagerAsync.ExecuteNonQueryAsync("UPDATE ayuda_release\n                  SET release_status = 'CANCELLED', notes = @notes\n                  WHERE release_id = @releaseId\n                    AND UPPER(COALESCE(release_status, 'RELEASED')) = 'RELEASED'", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@releaseId", (object)releaseId);
			cmd.Parameters.AddWithValue("@notes", (object)(before.Notes + "\nReversed: " + cleanReason).Trim());
		}).ConfigureAwait(continueOnCapturedContext: false);
		if (before.BatchId > 0)
		{
		...
		AuditTrailService.Log("Ayuda", "ayuda_release", releaseId, "REVERSE", before,
			new { ReleaseStatus = "CANCELLED", Reason = cleanReason },
			"Ayuda release reversed: " + cleanReason);
	}

	public Task DeleteReleaseAsync(int releaseId, string reason)
	{
		return CancelReleaseAsync(releaseId, reason);
	}
	...
	private async Task<AyudaBatchReleaseResult> SaveBatchReleaseOnlineAsync(int programId, DateTime releasedAt, string notes, IReadOnlyList<AyudaBeneficiaryDraft> beneficiaries, string batchReference, decimal totalAmount)
	{
		...
		try
		{
			...
			try
			{
				...
				try
				{
					MySqlCommand budgetLock = new MySqlCommand(
						"SELECT IFNULL(ap.allocated_budget, 0.00) - " +
						"IFNULL((SELECT SUM(ar.amount) FROM ayuda_release ar " +
						"WHERE ar.program_id = ap.program_id AND UPPER(COALESCE(ar.release_status, 'RELEASED')) <> 'CANCELLED'), 0.00) " +
						"FROM ayuda_program ap WHERE ap.program_id = @programId AND ap.barangay_id = @barangayId " +
						"AND UPPER(COALESCE(ap.status, 'ACTIVE')) = 'ACTIVE' FOR UPDATE",
						conn,
						tx);
					budgetLock.Parameters.AddWithValue("@programId", programId);
					budgetLock.Parameters.AddWithValue("@barangayId", ResolveBarangayId());
					object? availableValue = await budgetLock.ExecuteScalarAsync().ConfigureAwait(false);
					budgetLock.Dispose();
					if (availableValue == null || availableValue == DBNull.Value)
					{
						throw new InvalidOperationException("The selected active ayuda program could not be locked for posting.");
					}
					decimal availableBudget = Convert.ToDecimal(availableValue, CultureInfo.InvariantCulture);
					if (totalAmount > availableBudget)
					{
						throw new InvalidOperationException(
							$"The remaining budget changed. Available now: PHP {availableBudget:N2}. Review the distribution again.");
					}
	...
	{
		if (!OfflineDatabaseSupport.IsOffline && !DBConnection.IsSqliteSelected())
		{
