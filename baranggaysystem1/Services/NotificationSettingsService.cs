...
{
    private static readonly byte[] SecretEntropy =
        Encoding.UTF8.GetBytes("BarangaySystem.NotificationSettings.v1");
    ...
    public static SmtpSettings LoadSmtpSettings()
    {
        return new SmtpSettings
        {
            ...
            Password = GetSecretWithEnvFallback(KeySmtpPassword, "BARANGAY_SMTP_PASS"),
    ...
    public static void SaveSmtpSettings(SmtpSettings settings)
    {
        ...
        AuthorizationGuard.RequirePermission(PermissionKeys.OpenSettings, "change notification settings");
        ...
        SaveSecret(KeySmtpPassword, settings.Password);
    ...
    public static SmsSettings LoadSmsSettings()
    {
        return new SmsSettings
        {
            ...
            ApiToken = GetSecretWithEnvFallback(KeySmsApiToken, "BARANGAY_SMS_API_TOKEN"),
    ...
    public static void SaveSmsSettings(SmsSettings settings)
    {
        ...
        SaveSecret(KeySmsApiToken, settings.ApiToken);
    ...
    {
        if (settings == null) throw new ArgumentNullException(nameof(settings));
        AuthorizationGuard.RequirePermission(PermissionKeys.OpenSettings, "change reminder settings");
    ...
    private static string GetSecretWithEnvFallback(string configKey, string envKey)
    {
        string stored = SystemConfigService.Get(configKey);
        if (!string.IsNullOrWhiteSpace(stored))
        {
            if (stored.StartsWith("enc:", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                    byte[] protectedBytes = Convert.FromBase64String(stored.Substring(4));
                    byte[] plainBytes = ProtectedData.Unprotect(
                        protectedBytes, SecretEntropy, DataProtectionScope.CurrentUser);
                    return Encoding.UTF8.GetString(plainBytes);
                }
                catch (Exception ex)
                {
                    AppLogger.LogError($"Unable to decrypt protected setting '{configKey}'.", ex);
                    return string.Empty;
                }
            }

            // Migrate legacy plaintext the first time it is read.
            SaveSecret(configKey, stored);
            return stored;
        }
    ...
    private static void SaveSecret(string configKey, string? value)
    {
        string plain = (value ?? string.Empty).Trim();
        if (plain.Length == 0)
        {
            // A blank editor means "keep the current secret", not erase it.
            return;
        }

        byte[] protectedBytes = ProtectedData.Protect(
            Encoding.UTF8.GetBytes(plain), SecretEntropy, DataProtectionScope.CurrentUser);
        SystemConfigService.Set(configKey, "enc:" + Convert.ToBase64String(protectedBytes));
    }
