...
public static class SystemConfigService
{
	...
	public static bool HasCustomLogo()
	{
		try
		{
	...
	public static BitmapImage? GetDefaultLogo()
	{
		try
		{
			Uri uri = new Uri("pack://application:,,,/baranggaysystem1;component/Resources/app_logo.png", UriKind.Absolute);
			var streamInfo = Application.GetResourceStream(uri);
			if (streamInfo != null)
			{
				using MemoryStream ms = new MemoryStream();
				streamInfo.Stream.CopyTo(ms);
				return CreateBitmapImage(ms.ToArray());
			}
		}
		catch
		{
			try
			{
				string text = Path.Combine(AppContext.BaseDirectory, "Resources", "app_logo.png");
				if (File.Exists(text))
				{
					return CreateBitmapImage(File.ReadAllBytes(text));
				}
			}
			catch { }
		}
