...
internal sealed class EmergencyContactService
{
    ...
    {
        AuthorizationGuard.RequireAdmin("create emergency contacts");
    ...
    {
        AuthorizationGuard.RequireAdmin("update emergency contacts");
    ...
    public async Task SetActiveAsync(int contactId, bool isActive, string? reason = null)
    {
        AuthorizationGuard.RequireAdmin("change emergency contact status");
        DataTable existing = await GetAsync(contactId).ConfigureAwait(false);
        if (existing.Rows.Count == 0)
            throw new InvalidOperationException("The selected emergency contact no longer exists.");
        string? cleanReason = string.IsNullOrWhiteSpace(reason) ? null : reason.Trim();
        if (!isActive && (cleanReason == null || cleanReason.Length < 5))
        ...
        await DatabaseManagerAsync.ExecuteNonQueryAsync(
            "UPDATE emergency_contact SET is_active = @active WHERE contact_id = @id",
            cmd =>
            {
                cmd.Parameters.AddWithValue("@id", (object)contactId);
                cmd.Parameters.AddWithValue("@active", (object)(isActive ? 1 : 0));
            }).ConfigureAwait(false);
        AuditTrailService.Log("EmergencyContact", "emergency_contact", contactId,
            isActive ? "REACTIVATE" : "DEACTIVATE", null,
            new { isActive, reason = cleanReason },
            isActive ? "Emergency contact reactivated." : "Emergency contact deactivated: " + cleanReason);
    }
