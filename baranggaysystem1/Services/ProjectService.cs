...
internal sealed class ProjectService
{
	...
	private static readonly string[] AllowedStatuses = new string[5] { "Planned", "Ongoing", "On hold", "Completed", "Archived" };
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageProjects, "create projects and programs");
	...
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageProjects, "update projects and programs");
		...
		{
			throw new InvalidOperationException("Archived initiatives are locked and cannot be edited.");
		}
		ProjectRecord sanitized = Sanitize(record);
	...
	{
		await ArchiveProjectAsync(projectId, "Archived from the initiatives workflow.").ConfigureAwait(false);
	}

	public async Task ArchiveProjectAsync(int projectId, string reason)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.ManageProjects, "archive projects and programs");
		...
		if (string.Equals(before.Status, "Archived", StringComparison.OrdinalIgnoreCase))
		{
			throw new InvalidOperationException("This initiative is already archived.");
		}
		await DatabaseManagerAsync.ExecuteNonQueryAsync("UPDATE projects SET status = 'Archived' WHERE project_id = @projectId", delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@projectId", (object)projectId);
		}).ConfigureAwait(continueOnCapturedContext: false);
		AuditTrailService.Log("Projects", "project", projectId, "ARCHIVE", before,
			new { status = "Archived", reason = cleanReason }, "Initiative archived: " + cleanReason);
	}
