...
public class BarangayOfficialService
{
	...
	{
		AuthorizationGuard.RequireAdmin("assign barangay officials");
		...
		AuditTrailService.Log("Officials", "barangay_official", official.ResidentId, "CREATE",
			null, new { official.ResidentId, TermId = targetTermId, official.Position, official.Committee },
			"Barangay official assignment created.");
	}

	public async Task UpdateBarangayOfficialAsync(BarangayOfficial official)
	{
		AuthorizationGuard.RequireAdmin("update barangay officials");
		...
		AuditTrailService.Log("Officials", "barangay_official", official.OfficialId, "UPDATE",
			null, new { official.ResidentId, TermId = targetTermId, official.Position, official.Committee, official.Status },
			"Barangay official assignment updated.");
	}

	public async Task SetBarangayOfficialStatusAsync(int officialId, string status, string? reason = null)
	{
		AuthorizationGuard.RequireAdmin("change barangay official status");
		...
		AuditTrailService.Log("Officials", "barangay_official", officialId, "STATUS_CHANGE",
			null, new { Status = normalizedStatus, Reason = cleanReason },
			"Barangay official status changed.");
	}

	public async Task DeleteBarangayOfficialAsync(int officialId)
	{
		await SetBarangayOfficialStatusAsync(
			officialId, "INACTIVE", "Assignment archived instead of permanently deleted.");
	}
