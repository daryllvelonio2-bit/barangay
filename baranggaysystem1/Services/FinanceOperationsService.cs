...
internal sealed class FinanceOperationsService
{
	...
	{
		AuthorizationGuard.RequireAdmin("manage the expense ledger");
		ExpenseEntryRecord sanitized = Sanitize(record);
		bool isUpdate = sanitized.ExpenseId > 0;
		...
		AuditTrailService.Log("Finance", "expense_entry",
			isUpdate ? (object)sanitized.ExpenseId : sanitized.ExpenseTitle,
			isUpdate ? "UPDATE" : "CREATE", null, sanitized,
			isUpdate ? "Expense entry updated." : "Expense entry recorded.");
	}

	public async Task SaveInventoryItemAsync(InventoryItemRecord record)
	{
		AuthorizationGuard.RequireAdmin("manage inventory");
		InventoryItemRecord sanitized = Sanitize(record);
		bool isUpdate = sanitized.ItemId > 0;
		...
		AuditTrailService.Log("Inventory", "inventory_item",
			isUpdate ? (object)sanitized.ItemId : sanitized.ItemName,
			isUpdate ? "UPDATE" : "CREATE", null, sanitized,
			isUpdate ? "Inventory item updated." : "Inventory item created.");
	}

	public async Task SaveAssetAsync(AssetRecord record)
	{
		AuthorizationGuard.RequireAdmin("manage barangay assets");
		AssetRecord sanitized = Sanitize(record);
		bool isUpdate = sanitized.AssetId > 0;
		...
		AuditTrailService.Log("Assets", "asset_record",
			isUpdate ? (object)sanitized.AssetId : sanitized.AssetName,
			isUpdate ? "UPDATE" : "CREATE", null, sanitized,
			isUpdate ? "Asset record updated." : "Asset record created.");
	}

	public async Task SaveProcurementRequestAsync(ProcurementRequestRecord record)
	{
		AuthorizationGuard.RequireAdmin("manage procurement requests");
		...
		if (sanitized.ProcurementId > 0)
		{
			...
			if (string.Equals(procurementRequestRecord.WorkflowStatus, "RECEIVED", StringComparison.OrdinalIgnoreCase) ||
				string.Equals(procurementRequestRecord.WorkflowStatus, "CANCELLED", StringComparison.OrdinalIgnoreCase))
				throw new InvalidOperationException("Completed and cancelled procurement requests are locked.");
			sanitized.WorkflowStatus = NormalizeDefault(procurementRequestRecord.WorkflowStatus, "DRAFT");
		...
		{
			sanitized.WorkflowStatus = "DRAFT";
			string newApprovedByName = string.Empty;
			DateTime? newApprovedAt = null;
		...
		AuditTrailService.Log("Procurement", "procurement_request",
			sanitized.ProcurementId > 0 ? (object)sanitized.ProcurementId : sanitized.RequestTitle,
			sanitized.ProcurementId > 0 ? "UPDATE" : "CREATE", null, sanitized,
			sanitized.ProcurementId > 0 ? "Procurement request details updated." : "Procurement request created as draft.");
	}

	public async Task AdvanceProcurementWorkflowAsync(int procurementId)
	{
		AuthorizationGuard.RequireAdmin("advance procurement requests");
		...
		AuditTrailService.Log("Procurement", "procurement_request", procurementId, "STATUS_CHANGE",
			new { Status = procurementRequestRecord.WorkflowStatus }, new { Status = nextStatus },
			$"Procurement request advanced to {nextStatus}.");
	}

	public async Task CancelProcurementWorkflowAsync(int procurementId, string reason)
	{
		AuthorizationGuard.RequireAdmin("cancel procurement requests");
		...
		int affected = await DatabaseManagerAsync.ExecuteNonQueryAsync(@"
			...
			WHERE procurement_id = @procurementId
			  ...
			  AND UPPER(COALESCE(workflow_status, 'DRAFT')) NOT IN ('RECEIVED', 'CANCELLED')",
			cmd =>
			{
				cmd.Parameters.AddWithValue("@reason", cleanReason);
				cmd.Parameters.AddWithValue("@userId", GetUserIdOrNull());
				cmd.Parameters.AddWithValue("@procurementId", procurementId);
				cmd.Parameters.AddWithValue("@barangayId", GetBarangayId());
			}).ConfigureAwait(false);
		if (affected != 1)
			throw new DBConcurrencyException("Only active procurement requests can be cancelled.");
		AuditTrailService.Log("Procurement", "procurement_request", procurementId, "CANCEL", null,
			new { Status = "CANCELLED", Reason = cleanReason }, "Procurement request cancelled.");
	}
