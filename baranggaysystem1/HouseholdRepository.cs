...
internal sealed class HouseholdRepository
{
	...
	public HouseholdEditRecord? GetForEdit(int householdId, int barangayId)
	{
		...
		try
		{
			return GetForEditOnline(householdId, barangayId);
		}
		catch (Exception exception) when (TryActivateOfflineFallback(exception, "GetForEdit"))
		{
			return GetForEditOffline(householdId, barangayId);
		}
	}

	private HouseholdEditRecord? GetForEditOffline(int householdId, int barangayId)
	{
		DataTable dt = DbHelper.LoadTable("SELECT household_id, barangay_id, purok_id, house_no, street, subdivision, address_note, latitude, longitude FROM household WHERE household_id = @householdId AND barangay_id = @barangayId LIMIT 1", delegate(MySqlCommand cmd)
		{
		...
		return new HouseholdEditRecord
		{
			...
			BarangayId = Convert.ToInt32(row["barangay_id"]),
			PurokId = Convert.ToInt32(row["purok_id"]),
			HouseNo = (Convert.ToString(row["house_no"]) ?? string.Empty),
			Street = (Convert.ToString(row["street"]) ?? string.Empty),
			Subdivision = (Convert.ToString(row["subdivision"]) ?? string.Empty),
			AddressNote = (Convert.ToString(row["address_note"]) ?? string.Empty),
	...
	private HouseholdEditRecord? GetForEditOnline(int householdId, int barangayId)
	{
		MySqlConnection connection = DBConnection.GetConnection();
		try
		{
	...
	public int Create(HouseholdSaveRequest request)
	{
		...
		catch (Exception exception) when (TryActivateOfflineFallback(exception, "Create"))
		{
			// Offline create via DbHelper
			DbHelper.ExecuteNonQuery("INSERT INTO household (barangay_id, purok_id, house_no, street, subdivision, address_note, latitude, longitude) VALUES (@barangayId, @purokId, @houseNo, @street, @subdivision, @addressNote, @lat, @lng)", delegate(MySqlCommand cmd)
			{
				cmd.Parameters.AddWithValue("@barangayId", (object)num);
				cmd.Parameters.AddWithValue("@purokId", (object)request.PurokId);
				cmd.Parameters.AddWithValue("@houseNo", (object)(request.HouseNo ?? string.Empty));
				cmd.Parameters.AddWithValue("@street", (object)(request.Street ?? string.Empty));
				cmd.Parameters.AddWithValue("@subdivision", (object)(request.Subdivision ?? string.Empty));
				cmd.Parameters.AddWithValue("@addressNote", (object)(request.AddressNote ?? string.Empty));
				cmd.Parameters.AddWithValue("@lat", request.Latitude.HasValue ? (object)request.Latitude.Value : DBNull.Value);
				cmd.Parameters.AddWithValue("@lng", request.Longitude.HasValue ? (object)request.Longitude.Value : DBNull.Value);
			});
			DataTable dt = DbHelper.LoadTable("SELECT last_insert_rowid() AS id", null);
	...
	public void Update(int householdId, HouseholdSaveRequest request)
	{
		...
		catch (Exception exception) when (TryActivateOfflineFallback(exception, "Update"))
		{
			DbHelper.ExecuteNonQuery("UPDATE household SET purok_id = @purokId, house_no = @houseNo, street = @street, subdivision = @subdivision, address_note = @addressNote, latitude = @latitude, longitude = @longitude, updated_at = CURRENT_TIMESTAMP WHERE household_id = @householdId AND barangay_id = @barangayId", delegate(MySqlCommand cmd)
			{
				cmd.Parameters.AddWithValue("@purokId", (object)request.PurokId);
				cmd.Parameters.AddWithValue("@houseNo", (object)(request.HouseNo ?? string.Empty));
				cmd.Parameters.AddWithValue("@street", (object)(request.Street ?? string.Empty));
				cmd.Parameters.AddWithValue("@subdivision", (object)(request.Subdivision ?? string.Empty));
				cmd.Parameters.AddWithValue("@addressNote", (object)(request.AddressNote ?? string.Empty));
				cmd.Parameters.AddWithValue("@latitude", request.Latitude.HasValue ? (object)request.Latitude.Value : DBNull.Value);
				cmd.Parameters.AddWithValue("@longitude", request.Longitude.HasValue ? (object)request.Longitude.Value : DBNull.Value);
				cmd.Parameters.AddWithValue("@householdId", (object)householdId);
				cmd.Parameters.AddWithValue("@barangayId", (object)num);
			});
		}
	...
	public async Task ArchiveAsync(int householdId, int barangayId, string reason)
	{
		AuthorizationGuard.RequirePermission(PermissionKeys.DeleteHouseholds, "archive household records");
		...
		int resolvedBarangayId = ResolveBarangayId(barangayId);
		await using DatabaseTransactionScope tx = await DatabaseTransactionScope.BeginAsync().ConfigureAwait(false);
		int members = await tx.ExecuteScalarAsync<int>(
			"SELECT COUNT(*) FROM resident WHERE household_id = @householdId AND IFNULL(is_deleted, 0) = 0",
			new Dictionary<string, object?> { ["@householdId"] = householdId }).ConfigureAwait(false);
		if (members > 0)
		{
			throw new InvalidOperationException("Transfer all household members before archiving this household.");
		}
		int affected = await tx.ExecuteNonQueryAsync(@"
			...
			WHERE household_id = @householdId AND barangay_id = @barangayId
		...
		if (affected != 1)
		{
			throw new InvalidOperationException("The household is unavailable or already archived.");
		}
		await tx.CommitAsync().ConfigureAwait(false);
		AuditTrailService.Log("Households", "household", householdId, "ARCHIVE", null,
			new { Status = "ARCHIVED", Reason = cleanReason }, "Household archived.");
	}
	...
	public bool ExistsDuplicateAddress(int barangayId, int purokId, string? houseNo, string? street, int? excludeHouseholdId = null)
	{
		try
		{
			return ExistsDuplicateAddressOnline(barangayId, purokId, houseNo, street, excludeHouseholdId);
		}
		catch (Exception exception) when (TryActivateOfflineFallback(exception, "ExistsDuplicateAddress"))
		{
			// In offline mode, use DbHelper
			int num = ResolveBarangayId(barangayId);
			string text = (houseNo ?? string.Empty).Trim();
			string text2 = (street ?? string.Empty).Trim();
			DataTable dt = DbHelper.LoadTable("SELECT COUNT(*) AS cnt FROM household WHERE barangay_id = @barangayId AND purok_id = @purokId AND UPPER(TRIM(COALESCE(house_no, ''))) = UPPER(@houseNo) AND UPPER(TRIM(COALESCE(street, ''))) = UPPER(@street) AND (@excludeId IS NULL OR household_id <> @excludeId)", delegate(MySqlCommand cmd)
			{
				cmd.Parameters.AddWithValue("@barangayId", (object)num);
				cmd.Parameters.AddWithValue("@purokId", (object)purokId);
				cmd.Parameters.AddWithValue("@houseNo", (object)text);
				cmd.Parameters.AddWithValue("@street", (object)text2);
	...
	private bool ExistsDuplicateAddressOnline(int barangayId, int purokId, string? houseNo, string? street, int? excludeHouseholdId)
	{
	...
	public IReadOnlyList<ResidentPickerItem> GetResidentsForHouseholdPicker(int barangayId, int targetHouseholdId, string? searchText)
	{
		...
		DataTable dt = DbHelper.LoadTable(sql, delegate(MySqlCommand cmd)
		{
			cmd.Parameters.AddWithValue("@barangayId", (object)num);
			cmd.Parameters.AddWithValue("@targetHouseholdId", (object)targetHouseholdId);
			cmd.Parameters.AddWithValue("@searchText", (object)text);
			cmd.Parameters.AddWithValue("@searchLike", (object)text2);
		});
		List<ResidentPickerItem> list = new List<ResidentPickerItem>();
		foreach (DataRow row in dt.Rows)
		{
			string houseNo = Convert.ToString(row["house_no"]) ?? string.Empty;
			string street = Convert.ToString(row["street"]) ?? string.Empty;
			string subdivision = Convert.ToString(row["subdivision"]) ?? string.Empty;
			string purokName = Convert.ToString(row["purok_name"]) ?? string.Empty;
			list.Add(new ResidentPickerItem
			{
				...
				CurrentHouseholdId = (row["household_id"] == DBNull.Value ? (int?)null : Convert.ToInt32(row["household_id"])),
				CurrentPurokId = (row["purok_id"] == DBNull.Value ? (int?)null : Convert.ToInt32(row["purok_id"])),
	...
	private static bool IsConnectivityFailure(Exception exception)
	{
		for (Exception ex = exception; ex != null; ex = ex.InnerException)
		{
			...
			// Treat "SQLite mode is active" as a connectivity failure so offline fallback triggers
			if (ex is InvalidOperationException && ex.Message != null &&
				ex.Message.Contains("SQLite mode is active", StringComparison.OrdinalIgnoreCase))
			{
				return true;
			}
