...
internal static class GlobalSearchService
{
	internal static List<GlobalSearchResult> Search(string query, GlobalSearchScope scope, int limitPerType = 12)
	{
		...
		if (scope == GlobalSearchScope.All || scope == GlobalSearchScope.Payments)
		{
			list.AddRange(SearchPayments(like, flag ? new int?(result) : null, num));
		}
	...
	private static IEnumerable<GlobalSearchResult> SearchPayments(string like, int? idValue, int limit)
	{
		string generalSql = $@"SELECT payment_id, or_number, resident_name, amount, payment_date
			FROM payment_ledger
			WHERE barangay_id = @barangayId
			  AND (or_number LIKE @q OR resident_name LIKE @q{(idValue.HasValue ? " OR payment_id = @id" : string.Empty)})
			ORDER BY payment_date DESC, payment_id DESC
			LIMIT {limit}";
		DataTable general = DbHelper.LoadTable(generalSql, cmd =>
		{
			cmd.Parameters.AddWithValue("@barangayId", UserSession.BarangayId > 0 ? UserSession.BarangayId : 1);
		...
		{
			int id = ReadInt(row, "payment_id");
			string orNumber = Convert.ToString(row["or_number"]) ?? string.Empty;
			string resident = Convert.ToString(row["resident_name"]) ?? string.Empty;
			decimal amount = row["amount"] == DBNull.Value ? 0m : Convert.ToDecimal(row["amount"]);
			yield return new GlobalSearchResult(
				GlobalSearchEntityType.Payment, id,
				string.IsNullOrWhiteSpace(orNumber) ? $"Payment #{id}" : $"OR {orNumber}",
				$"{resident} | PHP {amount:N2}");
		}

		string documentSql = $@"SELECT dp.payment_id, COALESCE(dp.or_no, dr.or_number, '') AS or_number,
			       r.first_name, r.middle_name, r.last_name, IFNULL(dp.amount, dr.fee) AS amount,
			       dp.paid_at, dr.resident_id
			...
			WHERE dr.barangay_id = @barangayId
			  AND (COALESCE(dp.or_no, dr.or_number, '') LIKE @q
			       OR r.first_name LIKE @q OR r.middle_name LIKE @q OR r.last_name LIKE @q
			       {(idValue.HasValue ? " OR dp.payment_id = @id" : string.Empty)})
			ORDER BY dp.paid_at DESC, dp.payment_id DESC
			LIMIT {limit}";
		DataTable documents = DbHelper.LoadTable(documentSql, cmd =>
		{
			cmd.Parameters.AddWithValue("@barangayId", UserSession.BarangayId > 0 ? UserSession.BarangayId : 1);
			cmd.Parameters.AddWithValue("@q", like);
			if (idValue.HasValue)
		...
		foreach (DataRow row in documents.Rows)
		{
			...
			yield return new GlobalSearchResult(
				GlobalSearchEntityType.Payment, id,
				string.IsNullOrWhiteSpace(orNumber) ? $"Document payment #{id}" : $"OR {orNumber}",
				$"{resident} | PHP {amount:N2}", ReadInt(row, "resident_id"));
		}
	}
