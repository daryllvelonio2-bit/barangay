...
internal static class RepeatRespondentService
{
	...
	public static RepeatRespondentBatch LoadCounts(IEnumerable<int> residentIds, IEnumerable<string> normalizedNamesAll, IEnumerable<string> normalizedNamesNullIdOnly)
	{
		...
		try
		{
			if (OfflineDatabaseSupport.IsOffline || DBConnection.IsSqliteSelected())
			{
				if (list.Count > 0)
				{
					LoadCountsByResidentIdCompat(list, repeatRespondentBatch.ByResidentId);
				}
				if (list2.Count > 0)
				{
					LoadCountsByNameCompat(list2, includeLinked: true, repeatRespondentBatch.ByNameAll);
				}
				if (list3.Count > 0)
				{
					LoadCountsByNameCompat(list3, includeLinked: false, repeatRespondentBatch.ByNameNullIdOnly);
				}
				return repeatRespondentBatch;
			}
	...
	private static void LoadCountsByResidentIdCompat(
		IReadOnlyList<int> ids,
		Dictionary<int, RepeatRespondentCounts> target)
	{
		string placeholders = string.Join(", ", Enumerable.Range(0, ids.Count).Select(index => "@rid" + index));
		DataTable table = DbHelper.LoadTable(
			@"SELECT respondent_resident_id AS rid,
			  ...
			  FROM case_record
			  WHERE respondent_resident_id IN (" + placeholders + @")
			  GROUP BY respondent_resident_id",
			command =>
			{
				for (int index = 0; index < ids.Count; index++)
				{
					command.Parameters.AddWithValue("@rid" + index, ids[index]);
				}
			});
		foreach (DataRow row in table.Rows)
		{
			if (row["rid"] != DBNull.Value)
			{
				target[Convert.ToInt32(row["rid"])] = ReadCounts(row);
			}
		}
	}

	private static void LoadCountsByNameCompat(
		IReadOnlyList<string> names,
		bool includeLinked,
		Dictionary<string, RepeatRespondentCounts> target)
	{
		string placeholders = string.Join(", ", Enumerable.Range(0, names.Count).Select(index => "@nm" + index));
		...
		DataTable table = DbHelper.LoadTable(
			@"SELECT UPPER(TRIM(respondent_name)) AS norm_name,
			  ...
			  WHERE respondent_name IS NOT NULL
			    AND respondent_name <> ''
			    AND UPPER(TRIM(respondent_name)) IN (" + placeholders + ")" + linkedClause + @"
			  GROUP BY UPPER(TRIM(respondent_name))",
			command =>
			{
				for (int index = 0; index < names.Count; index++)
				{
					command.Parameters.AddWithValue("@nm" + index, names[index]);
				}
			});
		foreach (DataRow row in table.Rows)
		{
			string name = NormalizeName(Convert.ToString(row["norm_name"]));
			if (!string.IsNullOrWhiteSpace(name))
			{
				target[name] = ReadCounts(row);
			}
		}
	}

	private static RepeatRespondentCounts ReadCounts(DataRow row)
	{
		int totalCases = row["total_cases"] == DBNull.Value ? 0 : Convert.ToInt32(row["total_cases"]);
		int activeCases = row["active_cases"] == DBNull.Value ? 0 : Convert.ToInt32(row["active_cases"]);
