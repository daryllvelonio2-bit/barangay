...
internal static class SchemaGuard
{
		...
		try
		{
			...
			try
			{
					List<string> list = new string[]
					{
						...
						"outbound_notification", "resident_transfer_history", "role_permission", "backup_run", "expense_entry", "inventory_item", "asset_record", "procurement_request", "resident_classification", "resident_death_record", "schema_migrations",
						"official_term", "password_reset", "security_question", "ayuda_program", "ayuda_release_batch", "ayuda_release", "payment_ledger", "payment_void"
			...
			try
			{
					(string, string)[] obj = new(string, string)[]
				{
					("resident", "is_deleted"),
					("resident", "photo"),
					...
					("procurement_request", "workflow_status"),
						...
						("resident_death_record", "record_status"),
						("household", "status"),
						("role", "is_active"),
						("ayuda_release_batch", "batch_reference"),
						("ayuda_release", "release_status"),
						("payment_ledger", "or_number"),
						("payment_void", "void_reason")
					};
			...
			try
			{
				EnsureBootstrapSchemaIfNeeded(val);
			}
			catch (Exception ex2)
			{
				startupHealthReport.Add("Migration files", StartupHealthLevel.Warning, ex2.Message);
	...
	private static void EnsureBootstrapSchemaIfNeeded(MySqlConnection conn)
	{
		if (HasCoreBootstrapTables(conn))
		{
			return;
		}
		AppLogger.LogWarning("Core MySQL tables are missing; reapplying the full bootstrap schema.");
		MigrationRunner.ApplyMigrationFile(conn, "20260517_full_mysql_schema.sql", force: true);
	}

	private static bool HasCoreBootstrapTables(MySqlConnection conn)
	{
		string[] array = new string[7] { "barangay", "purok_sitio", "role", "user_account", "household", "resident", "system_config" };
		for (int i = 0; i < array.Length; i++)
		{
			if (!TableExists(conn, array[i]))
			{
				return false;
			}
		}
	...
	private static void EnsureAppCompatColumns(MySqlConnection conn)
	{
		...
		(string, string, string)[] array = new(string, string, string)[94]
		{
			...
			("resident", "occupation", "ALTER TABLE resident ADD COLUMN occupation VARCHAR(255) NULL"),
			("resident", "education_level", "ALTER TABLE resident ADD COLUMN education_level VARCHAR(255) NULL"),
			("resident", "educational_attainment", "ALTER TABLE resident ADD COLUMN educational_attainment VARCHAR(255) NULL"),
			("resident", "nationality", "ALTER TABLE resident ADD COLUMN nationality VARCHAR(100) NULL"),
			("resident", "religion", "ALTER TABLE resident ADD COLUMN religion VARCHAR(100) NULL"),
			("resident", "blood_type", "ALTER TABLE resident ADD COLUMN blood_type VARCHAR(10) NULL"),
			("resident", "email_address", "ALTER TABLE resident ADD COLUMN email_address VARCHAR(255) NULL"),
			("resident", "place_of_birth", "ALTER TABLE resident ADD COLUMN place_of_birth VARCHAR(255) NULL"),
			("resident", "house_no", "ALTER TABLE resident ADD COLUMN house_no VARCHAR(100) NULL"),
			("resident", "street", "ALTER TABLE resident ADD COLUMN street VARCHAR(255) NULL"),
			("resident", "philhealth_no", "ALTER TABLE resident ADD COLUMN philhealth_no VARCHAR(50) NULL"),
			("resident", "sss_no", "ALTER TABLE resident ADD COLUMN sss_no VARCHAR(50) NULL"),
			("resident", "tin_no", "ALTER TABLE resident ADD COLUMN tin_no VARCHAR(50) NULL"),
			("resident", "voters_id_no", "ALTER TABLE resident ADD COLUMN voters_id_no VARCHAR(50) NULL"),
			("resident", "date_of_residency", "ALTER TABLE resident ADD COLUMN date_of_residency DATE NULL"),
			("resident", "household_relationship", "ALTER TABLE resident ADD COLUMN household_relationship VARCHAR(100) NULL"),
			...
			("document_type", "doc_type_id", "ALTER TABLE document_type ADD COLUMN doc_type_id INT NULL"),
			...
			("document_request", "doc_type_id", "ALTER TABLE document_request ADD COLUMN doc_type_id INT NULL"),
			...
			("household", "status", "ALTER TABLE household ADD COLUMN status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE'"),
			("household", "archived_at", "ALTER TABLE household ADD COLUMN archived_at DATETIME NULL"),
			("household", "archived_by_user_id", "ALTER TABLE household ADD COLUMN archived_by_user_id INT NULL"),
			("household", "archive_reason", "ALTER TABLE household ADD COLUMN archive_reason VARCHAR(255) NULL"),
			("role", "is_active", "ALTER TABLE role ADD COLUMN is_active TINYINT(1) NOT NULL DEFAULT 1"),
			("role", "archived_at", "ALTER TABLE role ADD COLUMN archived_at DATETIME NULL"),
			("role", "archived_by_user_id", "ALTER TABLE role ADD COLUMN archived_by_user_id INT NULL"),
			("role", "archive_reason", "ALTER TABLE role ADD COLUMN archive_reason VARCHAR(255) NULL")
		};
		...
		TryExecuteIgnore(conn, "UPDATE document_type SET doc_type_id = document_type_id WHERE doc_type_id IS NULL");
		TryExecuteIgnore(conn, "UPDATE document_request SET doc_type_id = document_type_id WHERE doc_type_id IS NULL");
		TryExecuteIgnore(conn, "UPDATE document_request SET document_type_id = doc_type_id WHERE document_type_id IS NULL");
	}
	...
	private static void EnsureAppCompatTables(MySqlConnection conn)
	{
		...
		if (!TableExists(conn, "resident_death_record"))
		{
			MySqlCommand deathTable = new MySqlCommand(@"
			...
			try
			{
				((DbCommand)(object)deathTable).ExecuteNonQuery();
			}
			finally
			{
				((IDisposable)deathTable)?.Dispose();
			}
		}
	...
	private static void EnsureAppCompatIndexes(MySqlConnection conn)
	{
		...
		TryExecuteIgnore(conn, "CREATE INDEX idx_death_record_barangay_status ON resident_death_record(barangay_id, record_status)");
		TryExecuteIgnore(conn, "CREATE INDEX idx_death_record_resident ON resident_death_record(resident_id, record_status)");
