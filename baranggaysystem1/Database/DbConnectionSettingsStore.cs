...
internal static class DbConnectionSettingsStore
{
	...
	{
		return BuildOptions(LoadSettingsOrDefault());
	}
	...
	public static DatabaseConnectionOption GetSelectedOptionOrDefault()
	{
		...
		IReadOnlyList<DatabaseConnectionOption> source = BuildOptions(settings);
	...
	public static void SaveSelectedProfile(string selectedProfileKey, DatabaseConnectionProfile? customProfile = null)
	{
		...
		{
			if (string.Equals(storedDatabaseConnectionSettings.SelectedProfileKey, "hostinger", StringComparison.OrdinalIgnoreCase))
			{
				storedDatabaseConnectionSettings.HostingerProfile = Normalize(customProfile);
			}
			else
			{
	...
	private static bool TryDeserializeSettings(string json, out StoredDatabaseConnectionSettings settings)
	{
		...
		try
		{
			...
			settings.HostingerProfile.Password = DecodePassword(settings.HostingerProfile.Password);
			return true;
		}
		catch
		{
	...
	private static void SaveSettings(StoredDatabaseConnectionSettings settings)
	{
		...
		StoredDatabaseConnectionSettings value = new StoredDatabaseConnectionSettings
		{
			...
			HostingerProfile = new DatabaseConnectionProfile
			{
				Server = storedDatabaseConnectionSettings.HostingerProfile.Server,
				Port = storedDatabaseConnectionSettings.HostingerProfile.Port,
				Database = storedDatabaseConnectionSettings.HostingerProfile.Database,
				Username = storedDatabaseConnectionSettings.HostingerProfile.Username,
				Password = EncodePassword(storedDatabaseConnectionSettings.HostingerProfile.Password),
				UseSsl = storedDatabaseConnectionSettings.HostingerProfile.UseSsl
			}
		};
	...
	private static IReadOnlyList<DatabaseConnectionOption> BuildOptions(StoredDatabaseConnectionSettings settings)
	{
		DatabaseConnectionProfile profile = Normalize(settings.CustomProfile);
		DatabaseConnectionProfile hostingerProfile = Normalize(settings.HostingerProfile);
		return new List<DatabaseConnectionOption>
		{
			...
			new DatabaseConnectionOption
			{
				...
				Description = "Use the Hostinger MySQL database. Connection values are stored encrypted for the current Windows user.",
				Profile = hostingerProfile
			},
			new DatabaseConnectionOption
			{
				Key = "sqlite",
				DisplayName = "SQLite Local File",
				Description = "Use the local SQLite database stored in your Windows app-data folder. The bundled database is copied there on first use.",
				Profile = DatabaseConnectionProfile.CreateDefault(),
				UsesSqlite = true,
	...
	private static StoredDatabaseConnectionSettings CreateDefaultSettings()
	{
		return new StoredDatabaseConnectionSettings
		{
			...
			HostingerProfile = DBConnection.GetBootstrapProfile()
		};
	}
	...
	private static StoredDatabaseConnectionSettings Normalize(StoredDatabaseConnectionSettings? settings)
	{
		...
		obj.HostingerProfile = Normalize(obj.HostingerProfile ?? DBConnection.GetBootstrapProfile());
		return obj;
	}
	...
	private static DatabaseConnectionProfile ResolveSelectedProfile(StoredDatabaseConnectionSettings settings)
	{
		...
		{
			"hostinger" => Normalize(settings.HostingerProfile), 
