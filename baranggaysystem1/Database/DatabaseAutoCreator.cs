...
internal static class DatabaseAutoCreator
{
    ...
    /// <summary>
    /// Ensures the target database exists on the server without applying schema migrations.
    /// Useful before restoring a full SQL backup archive into a brand new database.
    /// </summary>
    public static bool EnsureDatabaseShellExists(DatabaseConnectionProfile profile)
    {
        if (profile == null || string.IsNullOrWhiteSpace(profile.Database))
            return false;
        ...
        try
        {
            ...
            {
                CreateDatabase(serverConn, databaseName);
                AppLogger.LogInfo($"[DatabaseAutoCreator] Created database shell '{databaseName}'.");
            }
        ...
        {
            AppLogger.LogWarning($"[DatabaseAutoCreator] Could not create database shell '{databaseName}'.", ex);
            return false;
        }
    }
    ...
    public static bool TryEnsureReady(string connectionString, DatabaseConnectionProfile profile)
    {
        ...
        try
        {
            // First try connecting directly - if it works, make sure the schema is initialized too.
            using var conn = new MySqlConnection(connectionString);
            ((DbConnection)(object)conn).Open();
            SchemaGuard.EnsureDatabaseReady(connectionString, force: true);
            conn.Close();
            return true;
        }
