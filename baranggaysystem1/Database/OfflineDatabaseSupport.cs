...
internal static class OfflineDatabaseSupport
{
	...
	public static bool EnsureInitialised()
	{
		lock (_lock)
		{
			...
			try
			{
				...
				PrepareWritableDatabase(_dbPath);
	...
	{
		return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), AppFolder, RelativeDatabaseDirectory, DbFileName);
	}

	private static void PrepareWritableDatabase(string targetPath)
	{
		if (File.Exists(targetPath) && new FileInfo(targetPath).Length > 0L)
		{
			return;
		}
		string text = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), AppFolder, DbFileName);
		string text2 = Path.Combine(AppContext.BaseDirectory, RelativeDatabaseDirectory, RelativeSqliteDirectory, DbFileName);
		string? text3 = null;
		if (File.Exists(text) && new FileInfo(text).Length > 0L)
		{
			text3 = text;
		}
		else if (File.Exists(text2) && new FileInfo(text2).Length > 0L)
		{
			text3 = text2;
		}
		if (text3 == null)
		...
		File.Copy(text3, targetPath, overwrite: false);
		AppLogger.LogInfo("[Offline] Prepared writable SQLite database from: " + text3);
	}
	...
	private static void EnsureCompatibilitySchema(SqliteConnection conn)
	{
		...
		AddColumnIfMissing(conn, "expense_entry", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		AddColumnIfMissing(conn, "inventory_item", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		AddColumnIfMissing(conn, "asset_record", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		AddColumnIfMissing(conn, "procurement_request", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		CreateTableIfMissing(conn, "resident_death_record", @"
                CREATE TABLE resident_death_record (
                    death_record_id INTEGER PRIMARY KEY AUTOINCREMENT,
                    barangay_id INTEGER NOT NULL,
		...
		AddColumnIfMissing(conn, "resident_classification", "classification_type", "TEXT NOT NULL DEFAULT 'TAG'");
		AddColumnIfMissing(conn, "resident_classification", "classification_key", "TEXT NULL");
		AddColumnIfMissing(conn, "resident_classification", "description", "TEXT NULL");
		AddColumnIfMissing(conn, "resident_classification", "color_hex", "TEXT NULL");
		AddColumnIfMissing(conn, "resident_classification", "status", "TEXT NOT NULL DEFAULT 'ACTIVE'");
		AddColumnIfMissing(conn, "resident_classification", "is_system", "INTEGER NOT NULL DEFAULT 0");
		AddColumnIfMissing(conn, "resident_classification", "sort_order", "INTEGER NOT NULL DEFAULT 0");
		AddColumnIfMissing(conn, "resident_classification", "created_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "resident_classification", "updated_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "resident_classification", "created_at", "TEXT NULL");
		AddColumnIfMissing(conn, "resident_classification", "updated_at", "TEXT NULL");
		AddColumnIfMissing(conn, "resident_classification", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		...
		AddColumnIfMissing(conn, "household", "status", "TEXT NOT NULL DEFAULT 'ACTIVE'");
		AddColumnIfMissing(conn, "household", "archived_at", "TEXT NULL");
		AddColumnIfMissing(conn, "household", "archived_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "household", "archive_reason", "TEXT NULL");
		AddColumnIfMissing(conn, "role", "is_active", "INTEGER NOT NULL DEFAULT 1");
		AddColumnIfMissing(conn, "role", "archived_at", "TEXT NULL");
		AddColumnIfMissing(conn, "role", "archived_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "role", "archive_reason", "TEXT NULL");
		...
		AddColumnIfMissing(conn, "resident", "educational_attainment", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "nationality", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "religion", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "blood_type", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "email_address", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "place_of_birth", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "house_no", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "street", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "philhealth_no", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "sss_no", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "tin_no", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "voters_id_no", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "date_of_residency", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "household_relationship", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "photo_path", "TEXT NULL");
		...
		AddColumnIfMissing(conn, "resident", "date_registered", "TEXT NULL");
		AddColumnIfMissing(conn, "resident", "email", "TEXT NULL");
		...
		AddColumnIfMissing(conn, "audit_trail", "before_json", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "after_json", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "notes", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "action_by", "INTEGER NULL");
		AddColumnIfMissing(conn, "audit_trail", "before_data", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "after_data", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "remarks", "TEXT NULL");
		AddColumnIfMissing(conn, "audit_trail", "user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "audit_trail", "created_at", "TEXT NULL");
		AddColumnIfMissing(conn, "barangay_official", "term_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "barangay_official", "committee", "TEXT NULL");
		AddColumnIfMissing(conn, "barangay_official", "status", "TEXT NOT NULL DEFAULT 'ACTIVE'");
		AddColumnIfMissing(conn, "document_request", "doc_type_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "document_request", "requested_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "document_request", "approved_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "document_request", "released_by_user_id", "INTEGER NULL");
		AddColumnIfMissing(conn, "document_type", "doc_type_id", "INTEGER NULL");
		...
		AddColumnIfMissing(conn, "projects", "lead", "TEXT NULL");
		AddColumnIfMissing(conn, "projects", "remarks", "TEXT NULL");
		AddColumnIfMissing(conn, "ayuda_program", "sync_status", "TEXT NOT NULL DEFAULT 'synced'");
		AddColumnIfMissing(conn, "document_type", "fee_default", "REAL NOT NULL DEFAULT 0.00");
		CreateTableIfMissing(conn, "payment_ledger", "\n                CREATE TABLE payment_ledger (\n                    payment_id INTEGER PRIMARY KEY AUTOINCREMENT,\n                    barangay_id INTEGER NOT NULL DEFAULT 1,\n                    resident_id INTEGER NOT NULL,\n                    resident_name TEXT NOT NULL,\n                    amount REAL NOT NULL DEFAULT 0.00,\n                    or_number TEXT NOT NULL,\n                    payment_method TEXT NOT NULL DEFAULT 'Cash',\n                    remarks TEXT,\n                    payment_date TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,\n                    created_by_user_id INTEGER,\n                    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,\n                    sync_status TEXT NOT NULL DEFAULT 'synced'\n                );");
		...
		try
		{
			((DbCommand)(object)val).CommandText = "\n                UPDATE purok_sitio SET is_active = 1 WHERE is_active IS NULL;\n                UPDATE resident SET is_head_of_family = 0 WHERE is_head_of_family IS NULL;\n                UPDATE resident SET is_solo_parent = 0 WHERE is_solo_parent IS NULL;\n                UPDATE resident SET is_youth = 0 WHERE is_youth IS NULL;\n                UPDATE resident SET is_indigent = 0 WHERE is_indigent IS NULL;\n                UPDATE resident SET email_address = COALESCE(email_address, email) WHERE email_address IS NULL;\n                UPDATE resident SET email = COALESCE(email, email_address) WHERE email IS NULL;\n                UPDATE resident SET date_registered = COALESCE(date_registered, created_at) WHERE date_registered IS NULL;\n                UPDATE projects SET record_type = 'Project' WHERE record_type IS NULL OR TRIM(record_type) = '';\n                UPDATE projects SET attendance_target = 0 WHERE attendance_target IS NULL;\n                UPDATE projects SET attendance_count = 0 WHERE attendance_count IS NULL;\n                UPDATE projects SET outcome_status = 'Pending' WHERE outcome_status IS NULL OR TRIM(outcome_status) = '';\n                UPDATE projects SET outcome_summary = '' WHERE outcome_summary IS NULL;\n                UPDATE projects SET name = COALESCE(name, title) WHERE name IS NULL OR TRIM(name) = '';\n                UPDATE audit_trail SET action_at = COALESCE(action_at, created_at) WHERE action_at IS NULL;\n                UPDATE audit_trail SET before_json = COALESCE(before_json, before_data) WHERE before_json IS NULL;\n                UPDATE audit_trail SET after_json = COALESCE(after_json, after_data) WHERE after_json IS NULL;\n                UPDATE audit_trail SET notes = COALESCE(notes, remarks) WHERE notes IS NULL;\n                UPDATE audit_trail SET action_by = COALESCE(action_by, user_id) WHERE action_by IS NULL;\n                UPDATE document_request SET doc_type_id = COALESCE(doc_type_id, document_type_id) WHERE doc_type_id IS NULL;\n                UPDATE document_request SET document_type_id = COALESCE(document_type_id, doc_type_id) WHERE document_type_id IS NULL;\n                UPDATE document_request SET requested_by_user_id = COALESCE(requested_by_user_id, created_by_user_id) WHERE requested_by_user_id IS NULL;\n                UPDATE document_type SET doc_type_id = COALESCE(doc_type_id, document_type_id) WHERE doc_type_id IS NULL;\n                UPDATE barangay_official SET status = CASE WHEN is_active = 1 THEN 'ACTIVE' ELSE 'INACTIVE' END WHERE status IS NULL OR TRIM(status) = '';\n                UPDATE ayuda_release SET batch_reference = reference_no WHERE batch_reference IS NULL OR TRIM(batch_reference) = '';\n                UPDATE ayuda_release_batch SET report_file_path = '' WHERE report_file_path IS NULL;";
			((DbCommand)(object)val).ExecuteNonQuery();
			...
			CreateIndexIfMissing(conn, "idx_death_record_barangay_status", "CREATE INDEX idx_death_record_barangay_status ON resident_death_record (barangay_id, record_status);");
			CreateIndexIfMissing(conn, "idx_death_record_resident", "CREATE INDEX idx_death_record_resident ON resident_death_record (resident_id, record_status);");
			...
			CreateIndexIfMissing(conn, "idx_official_term_barangay", "CREATE INDEX idx_official_term_barangay ON official_term (barangay_id);");
			CreateIndexIfMissing(conn, "idx_password_reset_token", "CREATE INDEX idx_password_reset_token ON password_reset (reset_token);");
			CreateIndexIfMissing(conn, "idx_password_reset_user", "CREATE INDEX idx_password_reset_user ON password_reset (user_id);");
			CreateIndexIfMissing(conn, "idx_security_question_user", "CREATE INDEX idx_security_question_user ON security_question (user_id);");
			CreateIndexIfMissing(conn, "idx_payment_void_date", "CREATE INDEX idx_payment_void_date ON payment_void (voided_at);");
	...
	private static string ResolveSqliteScript(string fileName)
	{
		...
		try
		{
			var assembly = typeof(OfflineDatabaseSupport).Assembly;
