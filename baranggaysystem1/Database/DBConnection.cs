...
internal class DBConnection
{
	...
	private const string EnvHostingerConnection = "BARANGAY_HOSTINGER_CONNECTION";
	...
	{
		string connectionString = Environment.GetEnvironmentVariable(EnvHostingerConnection) ?? string.Empty;
		try
		{
		...
		uint port = 3306u;
		uint.TryParse(Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_PORT"), out port);
		if (port == 0)
		{
			port = 3306u;
		}
		bool useSsl = !string.Equals(Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_USE_SSL"), "false", StringComparison.OrdinalIgnoreCase);
		return new DatabaseConnectionProfile
		{
			Server = Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_SERVER") ?? "your-host.mysql.hostinger.com",
			Port = port,
			Database = Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_DATABASE") ?? "your_database",
			Username = Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_USERNAME") ?? "your_user",
			Password = Environment.GetEnvironmentVariable("BARANGAY_HOSTINGER_PASSWORD") ?? string.Empty,
			UseSsl = useSsl
		};
	}
	...
	{
		if (includeOfflineMode && (IsSqliteSelected() || OfflineDatabaseSupport.IsOffline))
		{
			return true;
		}
		lock (SyncRoot)
		{
			return lastConnectivityFailureUtc.HasValue && DateTime.UtcNow - lastConnectivityFailureUtc.Value < ConnectivityFailureCooldown;
		}
	}
	...
	{
		if (IsSqliteSelected() || OfflineDatabaseSupport.IsOffline)
		{
			throw new InvalidOperationException("SQLite mode is active. Use DatabaseManagerAsync or DbHelper for provider-neutral database operations.");
		}
