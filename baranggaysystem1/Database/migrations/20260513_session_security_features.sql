...
-- Add must_change_password flag to user_account (safe: checks if exists)
SET @col_exists = (SELECT COUNT(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'user_account' AND COLUMN_NAME = 'must_change_password');
SET @sql = IF(@col_exists = 0, 'ALTER TABLE user_account ADD COLUMN must_change_password TINYINT NOT NULL DEFAULT 0', 'SELECT 1');
