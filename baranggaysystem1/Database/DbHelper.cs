...
internal static class DbHelper
{
	...
	private static int ExecuteNonQueryOffline(string sql, Action<MySqlCommand>? configure)
	{
		...
		try
		{
			...
			try
			{
				...
				// A deliberately selected SQLite file is a standalone database,
				// not an offline replica. Queue only connectivity-fallback writes.
	...
	{
		return OfflineDatabaseSupport.IsOffline || DBConnection.IsSqliteSelected();
	}
	...
	private static bool IsConnectivityFailure(Exception exception)
	{
		for (Exception ex = exception; ex != null; ex = ex.InnerException)
		{
			...
			// Treat "SQLite mode is active" as connectivity failure so offline fallback triggers
