...
internal static class MigrationRunner
{
	...
	public static bool ApplyMigrationFile(MySqlConnection conn, string migrationFileName, bool force = false)
	{
		if (conn == null || string.IsNullOrWhiteSpace(migrationFileName))
		{
			return false;
		}
		EnsureMigrationsTable(conn);
		...
		string text2 = Path.Combine(text, migrationFileName);
		if (!File.Exists(text2))
		{
			return false;
		}
		if (!force && IsMigrationApplied(conn, migrationFileName))
		{
			return false;
		}
		string sql = File.ReadAllText(text2);
		if (IsManualMigration(sql))
		{
			return false;
		}
		ApplySqlScript(conn, migrationFileName, sql);
		if (!IsMigrationApplied(conn, migrationFileName))
		{
			MarkApplied(conn, migrationFileName);
		}
		return true;
	}
	...
	private static (int DateKey, int Priority, string Name) GetSortKey(string filePath)
	{
		...
		int item = 0;
		if (text.Contains("full_mysql_schema"))
		{
			item = int.MinValue;
		}
		else if (text.Contains("seed_demo_data"))
		{
			item = int.MinValue + 1;
		}
		else if (fileName.Length >= 8 && int.TryParse(fileName.Substring(0, 8), out var result))
		{
			item = result;
		}
		int item2;
		...
		else if (text.Contains("new_schema"))
		{
			item2 = 0;
		}
		...
		else if (text.Contains("role_permission"))
		{
			item2 = 2;
		}
		else if (text.Contains("backup_run"))
		{
			item2 = 3;
		}
		else if (text.Contains("add_indexes"))
		{
			item2 = 9;
		}
		else
	...
	private static bool IsMigrationApplied(MySqlConnection conn, string migrationName)
	{
		...
		HashSet<string> hashSet = LoadAppliedMigrations(conn);
		return hashSet.Contains(migrationName);
	}
