...
internal static class OfflineSyncService
{
	private sealed class QueueItem
	{
		...
		public string TableName { get; set; } = "unknown";
	...
	{
		return TrySyncPendingChangesDetailed().SyncedCount;
	}

	public static SyncReplayResult TrySyncPendingChangesDetailed()
	{
		//IL_005c: Unknown result type (might be due to invalid IL or missing references)
		//IL_0063: Expected O, but got Unknown
		if (OfflineDatabaseSupport.IsOffline)
		{
			return new SyncReplayResult(0, 0, GetPendingCount());
		}
		try
		{
			if (!OfflineDatabaseSupport.EnsureInitialised())
			{
				return new SyncReplayResult(0, 0, 0);
			}
			...
			try
			{
				...
				try
				{
					...
					foreach (QueueItem item in list)
					{
						try
						{
							...
							try
							{
								...
								int affected = ((DbCommand)(object)val).ExecuteNonQuery();
								if ((item.Operation == "UPDATE" || item.Operation == "DELETE") && affected == 0)
								{
									throw new DBConcurrencyException(
										$"Queued {item.Operation.ToLowerInvariant()} for {item.TableName} matched no server record.");
								}
					...
					return new SyncReplayResult(num, failed, GetQueueCount(connection));
				}
				finally
				{
		...
		catch (Exception ex2)
		{
			...
			return new SyncReplayResult(0, 1, GetPendingCount());
		}
	}

	public static int GetPendingCount()
	{
		return TryGetPendingCount(out int count, out _) ? count : 0;
	}

	public static bool TryGetPendingCount(out int count, out string? error)
	{
		count = 0;
		error = null;
		try
		{
			if (!OfflineDatabaseSupport.EnsureInitialised())
			{
				error = "The local database is unavailable.";
				return false;
			}
			using SqliteConnection connection = OfflineDatabaseSupport.GetConnection();
			count = GetQueueCount(connection);
			return true;
		}
		catch (Exception ex)
		{
			error = ex.Message;
			AppLogger.LogWarning("[OfflineSync] Pending count could not be read.", ex);
			return false;
		}
	}
	...
	public static int GetFailureCount()
	{
		try
		{
			...
			command.CommandText = "SELECT COUNT(*) FROM sync_queue WHERE IFNULL(retry_count, 0) > 0;";
			return Convert.ToInt32(command.ExecuteScalar() ?? 0);
		}
		catch (Exception ex)
		{
			AppLogger.LogWarning("[OfflineSync] Failure count could not be read.", ex);
			return 0;
		}
	}

	public static IReadOnlyDictionary<string, int> GetPendingSummary()
	{
		...
		using SqliteConnection connection = OfflineDatabaseSupport.GetConnection();
		using SqliteCommand command = connection.CreateCommand();
		command.CommandText = "SELECT COALESCE(NULLIF(table_name, ''), 'unknown'), COUNT(*) FROM sync_queue GROUP BY table_name ORDER BY table_name;";
	...
	private static List<QueueItem> LoadQueueItems(SqliteConnection conn)
	{
		...
		try
		{
			((DbCommand)(object)val).CommandText = "SELECT queue_id, table_name, operation, sql_text, parameter_json\r\nFROM sync_queue\r\nORDER BY queue_id ASC";
			...
			try
			{
				while (((DbDataReader)(object)val2).Read())
				{
					list.Add(new QueueItem
					{
						...
						TableName = (((DbDataReader)(object)val2).IsDBNull(1) ? "unknown" : ((DbDataReader)(object)val2).GetString(1)),
						Operation = (((DbDataReader)(object)val2).IsDBNull(2) ? "UNKNOWN" : ((DbDataReader)(object)val2).GetString(2)),
						SqlText = (((DbDataReader)(object)val2).IsDBNull(3) ? string.Empty : ((DbDataReader)(object)val2).GetString(3)),
						ParameterJson = (((DbDataReader)(object)val2).IsDBNull(4) ? null : ((DbDataReader)(object)val2).GetString(4))
					});
				}
				return list;
			}
	...
	private static int GetQueueCount(SqliteConnection connection)
	{
		using SqliteCommand command = connection.CreateCommand();
...
internal sealed record SyncReplayResult(int SyncedCount, int FailedCount, int RemainingCount);
