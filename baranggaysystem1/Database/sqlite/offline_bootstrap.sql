...
CREATE TABLE IF NOT EXISTS document_type (
    ...
    fee_default REAL NOT NULL DEFAULT 0.00,
    sync_status TEXT NOT NULL DEFAULT 'synced',
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS ayuda_program (
    ...
    category           TEXT NOT NULL DEFAULT 'Financial Assistance',
...
CREATE INDEX IF NOT EXISTS idx_ayuda_program_status ON ayuda_program (status);
CREATE INDEX IF NOT EXISTS idx_ayuda_program_barangay ON ayuda_program (barangay_id);
...
CREATE INDEX IF NOT EXISTS idx_ayuda_batch_program ON ayuda_release_batch (program_id);
...
CREATE TABLE IF NOT EXISTS ayuda_release (
    ...
    sync_status        TEXT NOT NULL DEFAULT 'synced',
    FOREIGN KEY (program_id) REFERENCES ayuda_program (program_id) ON DELETE CASCADE,
    FOREIGN KEY (batch_id) REFERENCES ayuda_release_batch (batch_id) ON DELETE SET NULL,
    FOREIGN KEY (resident_id) REFERENCES resident (resident_id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_ayuda_release_program ON ayuda_release (program_id);
CREATE INDEX IF NOT EXISTS idx_ayuda_release_batch ON ayuda_release (batch_id);
CREATE INDEX IF NOT EXISTS idx_ayuda_release_resident ON ayuda_release (resident_id);
...
CREATE INDEX IF NOT EXISTS idx_payment_resident ON payment_ledger (resident_id);
CREATE INDEX IF NOT EXISTS idx_payment_date ON payment_ledger (payment_date);
CREATE INDEX IF NOT EXISTS idx_payment_or ON payment_ledger (or_number);

-- Verified deceased-resident lifecycle
...
CREATE INDEX IF NOT EXISTS idx_death_record_barangay_status ON resident_death_record (barangay_id, record_status);
CREATE INDEX IF NOT EXISTS idx_death_record_resident ON resident_death_record (resident_id, record_status);
