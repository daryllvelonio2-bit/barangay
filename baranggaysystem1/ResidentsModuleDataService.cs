...
internal sealed class ResidentsModuleDataService
{
	...
	public async Task<DataTable> LoadHouseholdsAsync(string? searchText, int? purokId, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		DataTable archivedHouseholds = await DatabaseManagerAsync.LoadTableAsync(
			"SELECT household_id FROM household WHERE barangay_id = @barangayId AND UPPER(COALESCE(status, 'ACTIVE')) <> 'ACTIVE'",
			cmd => cmd.Parameters.AddWithValue("@barangayId", _barangayId),
			cancellationToken).ConfigureAwait(false);
		HashSet<int> archivedIds = archivedHouseholds.AsEnumerable()
			.Select(row => Convert.ToInt32(row["household_id"], CultureInfo.InvariantCulture))
			.ToHashSet();
		foreach (DataRow archivedRow in dataTable.AsEnumerable()
			.Where(row => archivedIds.Contains(Convert.ToInt32(row["household_id"], CultureInfo.InvariantCulture)))
			.ToList())
		{
			dataTable.Rows.Remove(archivedRow);
		}
	...
	public async Task<int> SaveResidentAsync(ResidentDto resident, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		bool useCompatibilitySave = OfflineDatabaseSupport.IsOffline ||
			DBConnection.IsSqliteSelected() ||
			DBConnection.ShouldThrottleOnlineAccess(includeOfflineMode: false);
	...
	{
		await ArchiveResidentAsync(residentId, reason, cancellationToken).ConfigureAwait(false);
	}

	public async Task ArchiveResidentAsync(int residentId, string? reason, CancellationToken cancellationToken = default(CancellationToken))
	{
		if (!Permissions.CanDeleteResidents)
		{
			throw new UnauthorizedAccessException("You do not have permission to archive resident records.");
		}
		if (residentId <= 0)
		{
		...
		if (OfflineDatabaseSupport.IsOffline || DBConnection.IsSqliteSelected())
		{
			if (!OfflineDatabaseSupport.IsAvailable && !OfflineDatabaseSupport.EnsureInitialised())
			{
				throw new InvalidOperationException("The local resident database is unavailable.");
			}
		}
		else
		{
		...
		string deleteReason = (reason ?? string.Empty).Trim();
		...
		AuditTrailService.Log("Residents", "resident", residentId, "ARCHIVE", new
		{
	...
	private int SaveResidentCore(ResidentDto resident, bool isUpdate)
	{
		...
		try
		{
			...
			try
			{
				...
				MySqlCommand val3 = new MySqlCommand("INSERT INTO resident\n                (barangay_id, purok_id, household_id, first_name, middle_name, last_name,\n                 suffix, sex, birth_date, civil_status, contact_no, is_pwd, is_senior, is_4ps_beneficiary,\n                 is_registered_voter, is_solo_parent, is_youth, is_indigent, status,\n                 occupation, educational_attainment, nationality, religion, blood_type, email_address,\n                 place_of_birth, house_no, street, philhealth_no, sss_no, tin_no, voters_id_no,\n                 date_of_residency, household_relationship, created_at, updated_at)\n              VALUES\n                (@barangayId, @purokId, @householdId, @firstName, @middleName, @lastName,\n                 @suffix, @sex, @birthDate, @civilStatus, @contactNo, @isPwd, @isSenior, @is4PsBeneficiary,\n                 @isRegisteredVoter, @isSoloParent, @isYouth, @isIndigent, @status,\n                 @occupation, @educationalAttainment, @nationality, @religion, @bloodType, @emailAddress,\n                 @placeOfBirth, @houseNo, @street, @philhealthNo, @sssNo, @tinNo, @votersIdNo,\n                 @dateOfResidency, @householdRelationship, NOW(), NOW())", connection, val);
				try
				{
	...
	private int SaveResidentCompatCore(ResidentDto resident, bool isUpdate)
	{
		...
		if (DbHelper.ExecuteNonQuery("INSERT INTO resident\n                (barangay_id, purok_id, household_id, first_name, middle_name, last_name,\n                 suffix, sex, birth_date, civil_status, contact_no, is_pwd, is_senior, is_4ps_beneficiary,\n                 is_registered_voter, is_solo_parent, is_youth, is_indigent, status,\n                 occupation, educational_attainment, nationality, religion, blood_type, email_address,\n                 place_of_birth, house_no, street, philhealth_no, sss_no, tin_no, voters_id_no,\n                 date_of_residency, household_relationship, created_at, updated_at)\n              VALUES\n                (@barangayId, @purokId, @householdId, @firstName, @middleName, @lastName,\n                 @suffix, @sex, @birthDate, @civilStatus, @contactNo, @isPwd, @isSenior, @is4PsBeneficiary,\n                 @isRegisteredVoter, @isSoloParent, @isYouth, @isIndigent, @status,\n                 @occupation, @educationalAttainment, @nationality, @religion, @bloodType, @emailAddress,\n                 @placeOfBirth, @houseNo, @street, @philhealthNo, @sssNo, @tinNo, @votersIdNo,\n                 @dateOfResidency, @householdRelationship, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)", delegate(MySqlCommand cmd)
		{
	...
	private void FillResidentCommand(MySqlCommand command, ResidentDto resident, int? residentId)
	{
		...
		command.Parameters.AddWithValue("@occupation", (object)(string.IsNullOrWhiteSpace(resident.Occupation) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.Occupation)));
		command.Parameters.AddWithValue("@educationalAttainment", (object)(string.IsNullOrWhiteSpace(resident.EducationalAttainment) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.EducationalAttainment)));
		command.Parameters.AddWithValue("@nationality", (object)(string.IsNullOrWhiteSpace(resident.Nationality) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.Nationality)));
		command.Parameters.AddWithValue("@religion", (object)(string.IsNullOrWhiteSpace(resident.Religion) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.Religion)));
		command.Parameters.AddWithValue("@bloodType", (object)(string.IsNullOrWhiteSpace(resident.BloodType) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.BloodType)));
		command.Parameters.AddWithValue("@emailAddress", (object)(string.IsNullOrWhiteSpace(resident.EmailAddress) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.EmailAddress)));
		command.Parameters.AddWithValue("@placeOfBirth", (object)(string.IsNullOrWhiteSpace(resident.PlaceOfBirth) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.PlaceOfBirth)));
		command.Parameters.AddWithValue("@houseNo", (object)(string.IsNullOrWhiteSpace(resident.HouseNo) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.HouseNo)));
		command.Parameters.AddWithValue("@street", (object)(string.IsNullOrWhiteSpace(resident.Street) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.Street)));
		command.Parameters.AddWithValue("@philhealthNo", (object)(string.IsNullOrWhiteSpace(resident.PhilHealthNo) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.PhilHealthNo)));
		command.Parameters.AddWithValue("@sssNo", (object)(string.IsNullOrWhiteSpace(resident.SssNo) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.SssNo)));
		command.Parameters.AddWithValue("@tinNo", (object)(string.IsNullOrWhiteSpace(resident.TinNo) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.TinNo)));
		command.Parameters.AddWithValue("@votersIdNo", (object)(string.IsNullOrWhiteSpace(resident.VotersIdNo) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.VotersIdNo)));
		command.Parameters.AddWithValue("@dateOfResidency", resident.DateOfResidency.HasValue ? ((object)resident.DateOfResidency.Value.Date) : DBNull.Value);
		command.Parameters.AddWithValue("@householdRelationship", (object)(string.IsNullOrWhiteSpace(resident.HouseholdRelationship) ? ((IConvertible)DBNull.Value) : ((IConvertible)resident.HouseholdRelationship)));
