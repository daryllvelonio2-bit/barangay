...
public partial class HouseholdsPage : UserControl, IRefreshable
{
	...
	private void MainGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
	{
		...
		{
			int memberCount = GetInt(dataRowView, "member_count");
			...
			selectedRecordMetaLabel.Text = $"{GetText(dataRowView, "purok_display", "No purok")} | {memberCount} member(s) | {GetText(dataRowView, "coordinates_display", "No coordinates saved")}";
			btnViewMembers.Visibility = memberCount > 0 ? Visibility.Visible : Visibility.Collapsed;
			btnTransfer.Visibility = memberCount > 0 ? Visibility.Visible : Visibility.Collapsed;
			btnDeleteHousehold.Visibility = memberCount == 0 ? Visibility.Visible : Visibility.Collapsed;
		}
		else
		{
			contextActionBar.Visibility = Visibility.Collapsed;
	...
	private async void BtnDelete_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			DialogService.Instance.ShowWarning("You do not have permission to archive household records.");
			return;
		}
		if (!(mainGrid.SelectedItem is DataRowView dataRowView))
		{
			DialogService.Instance.ShowWarning("Please select a household to archive.");
			return;
		}
		...
		var prompt = new ReasonPromptWindow(
			"Archive Household",
			$"Explain why household #{num} at {value} is being archived.",
			"Archive household")
		{
		...
		try
		{
			await _householdRepository.ArchiveAsync(num, _barangayId, prompt.Reason);
			mainGrid.SelectedItem = null;
			DialogService.Instance.ShowInfo("Household archived successfully.");
			await LoadAsync();
		}
		catch (Exception ex)
		{
			AppLogger.LogError("Household archive failed.", ex);
			DialogService.Instance.ShowError(ex.Message, "Archive Household");
		}
	}
	...
	private void BtnEdit_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			AppLogger.LogError("Failed to open household details.", ex);
			DialogService.Instance.ShowError("Could not open household details: " + ex.Message);
		}
	}
	...
	private void BtnTransferHistory_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			AppLogger.LogError("Failed to open transfer history.", ex);
			DialogService.Instance.ShowError("Could not open transfer history: " + ex.Message);
		}
	}
	...
	private void BtnEditDetails_Click(object sender, RoutedEventArgs e)
	{
		...
		else
		{
			...
			{
				AppLogger.LogError("Failed to open household editor.", ex);
				DialogService.Instance.ShowError("Could not open the household editor: " + ex.Message);
			}
		}
	}
