...
public partial class StaffManagementPage : UserControl, IRefreshable
{
	...
	private async void BtnDelete_Click(object sender, RoutedEventArgs e)
	{
		...
		var prompt = new ReasonPromptWindow(
			"Deactivate Staff",
			"Explain why " + text + " is being deactivated.",
			"Deactivate")
		{
		...
		try
		{
			await _staffService.SetStaffActiveStatusAsync(
				Convert.ToInt32(dataRowView["user_id"]),
				isActive: false,
				reason: prompt.Reason);
			DialogService.Instance.ShowInfo("Staff account deactivated. Its history and audit records were retained.");
			await LoadAsync();
		}
		catch (Exception ex)
		{
			AppLogger.LogError("Staff deactivation failed.", ex);
	...
	private void BtnResetPassword_Click(object sender, RoutedEventArgs e)
	{
		...
		int targetUserId = ((dataRowView["user_id"] != DBNull.Value) ? Convert.ToInt32(dataRowView["user_id"]) : 0);
