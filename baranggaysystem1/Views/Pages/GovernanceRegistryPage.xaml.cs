...
public partial class GovernanceRegistryPage : UserControl, IRefreshable
{
	...
	private async void BtnArchiveSelected_Click(object sender, RoutedEventArgs e)
	{
		if (!(GetActiveGrid().SelectedItem is DataRowView row))
		{
			DialogService.Instance.ShowWarning("Select a record to archive first.");
			return;
		}
		try
		{
			string? reason = DialogService.Instance.PromptForReason(
				"Archive Record",
				"Why is this record being archived? The reason will be retained in the audit trail.",
				"Archive");
			if (reason == null)
			{
				return;
			...
			if (_activeSection == GovernanceSection.Projects)
			{
				...
				if (DialogService.Instance.Confirm($"Archive {value} \"{projectRecord.Name}\"? The record will remain searchable and auditable.", "Archive Record"))
				{
					await _projectService.ArchiveProjectAsync(projectRecord.ProjectId, reason);
					DialogService.Instance.ShowInfo("Project or program archived.");
					await LoadAsync();
				}
			}
			else
			{
				...
				if (DialogService.Instance.Confirm("Archive announcement \"" + announcementRecord.Title + "\"? It will be removed from current feeds.", "Archive Announcement"))
				{
					await _announcementService.ArchiveAnnouncementAsync(announcementRecord.AnnouncementId, reason);
					DialogService.Instance.ShowInfo("Announcement archived.");
					await LoadAsync();
				}
			}
		}
		...
		{
			AppLogger.LogError("GovernanceRegistryPage archive failed.", ex);
			DialogService.Instance.ShowError(ex.Message, "Archive Record");
		}
	}
	...
	private void UpdateSelectionState(DataRowView? row)
	{
		...
		btnArchiveSelected.IsEnabled = !string.Equals(
			ReadString(row.Row, "status"), "Archived", StringComparison.OrdinalIgnoreCase);
		btnEditSelected.IsEnabled = btnArchiveSelected.IsEnabled;
