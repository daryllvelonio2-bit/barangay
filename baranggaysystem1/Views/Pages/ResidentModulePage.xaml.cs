...
public partial class ResidentModulePage : UserControl, IRefreshable
{
	...
	private void BtnContextBlotter_Click(object sender, RoutedEventArgs e)
	{
		if (mainGrid.SelectedItem is DataRowView dataRowView)
		{
			try
			{
				...
				var adapter = new DialogContentAdapter(blotterWindow);
				...
				NavigationService.Instance.NavigateToFullscreen(new FullscreenViewConfig
				{
					...
					Subtitle = $"File a blotter case for {residentName}",
			...
			{
				AppLogger.LogError("Failed to open blotter form.", ex);
				DialogService.Instance.ShowError("Could not open the blotter form: " + ex.Message);
			}
		}
	}
	...
	private void BtnContextBlotterHistory_Click(object sender, RoutedEventArgs e)
	{
		if (mainGrid.SelectedItem is DataRowView dataRowView)
		{
			try
			{
				...
				string residentName = dataRowView["full_name"]?.ToString() ?? "Resident";
				var historyWindow = new ResidentBlotterHistoryWindow(residentId, residentName);
				var adapter = new DialogContentAdapter(historyWindow);

				NavigationService.Instance.NavigateToFullscreen(new FullscreenViewConfig
				{
					Title = $"Blotter History - {residentName}",
					Subtitle = "View blotter cases for this resident",
					OriginRoute = "ResidentWorkspace",
					Content = adapter,
			...
			{
				AppLogger.LogError("Failed to open blotter history.", ex);
				DialogService.Instance.ShowError("Could not open blotter history: " + ex.Message);
			}
		}
	}
	...
	private void BtnContextCert_Click(object sender, RoutedEventArgs e)
	{
		if (mainGrid.SelectedItem is DataRowView dataRowView)
		{
			try
			{
				...
				var adapter = new DialogContentAdapter(certificationWindow);

				NavigationService.Instance.NavigateToFullscreen(new FullscreenViewConfig
				{
					Title = $"Issue Certificate - {residentName}",
					Subtitle = "Create a certificate or clearance for this resident",
					OriginRoute = "ResidentWorkspace",
					Content = adapter,
			...
			{
				AppLogger.LogError("Failed to open certificate form.", ex);
				DialogService.Instance.ShowError("Could not open certificate form: " + ex.Message);
			}
		}
	}
	...
	private void BtnContextHousehold_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			int residentId = Convert.ToInt32(dataRowView["resident_id"]);
			int? householdId = dataRowView["household_id"] == DBNull.Value
				? null
				: Convert.ToInt32(dataRowView["household_id"]);
			string residentName = dataRowView["full_name"]?.ToString() ?? "Resident";
			FullscreenDialogNavigator.Open(
				new HouseholdDetailsWindow(householdId, residentId),
				householdId.HasValue ? $"Household #{householdId}" : "Household Details",
				$"Family membership for {residentName}",
				"ResidentWorkspace",
				IconChar.Home,
				onSaved: RefreshData);
		}
	...
	private void BtnContextPayment_Click(object sender, RoutedEventArgs e)
	{
		if (mainGrid.SelectedItem is DataRowView dataRowView)
		{
			...
			string residentName = dataRowView["full_name"]?.ToString() ?? "Resident";
			FullscreenDialogNavigator.Open(
				new PaymentWindow(residentId, residentName),
				"Record Payment",
				residentName,
				"ResidentWorkspace",
				IconChar.MoneyBill,
	...
	private async void BtnDelete_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			DialogService.Instance.ShowWarning("You do not have permission to archive resident records.");
			return;
		}
		if (!(mainGrid.SelectedItem is DataRowView dataRowView))
		{
			DialogService.Instance.ShowWarning("Please select a resident to archive.");
			return;
		}
		int residentId = Convert.ToInt32(dataRowView["resident_id"]);
		...
		var prompt = new ReasonPromptWindow(
			"Archive Resident",
			"Explain why " + name + " is being removed from the active registry.",
			"Archive resident")
		{
		...
		try
		{
			await _dataService.ArchiveResidentAsync(residentId, prompt.Reason);
			mainGrid.UnselectAll();
		...
		{
			AppLogger.LogError("Resident archive failed.", ex);
			DialogService.Instance.ShowError(ex.Message, "Archive Resident");
		}
	}
	...
	private ResidentDto CreateResidentDto(DataRow row)
	{
		return new ResidentDto
		{
			...
			Occupation = GetCellText(row, "occupation"),
			EducationalAttainment = GetCellText(row, "educational_attainment"),
			Nationality = GetCellText(row, "nationality"),
			Religion = GetCellText(row, "religion"),
			BloodType = GetCellText(row, "blood_type"),
			EmailAddress = GetCellText(row, "email_address"),
			PlaceOfBirth = GetCellText(row, "place_of_birth"),
			HouseNo = GetCellText(row, "house_no"),
			Street = GetCellText(row, "street"),
			PhilHealthNo = GetCellText(row, "philhealth_no"),
			SssNo = GetCellText(row, "sss_no"),
			TinNo = GetCellText(row, "tin_no"),
			VotersIdNo = GetCellText(row, "voters_id_no"),
			DateOfResidency = ((row.Table.Columns.Contains("date_of_residency") && row["date_of_residency"] != DBNull.Value) ? new DateTime?(Convert.ToDateTime(row["date_of_residency"])) : null),
			HouseholdRelationship = GetCellText(row, "household_relationship")
		};
	}
	...
	private void BtnViewDetails_Click(object sender, RoutedEventArgs e)
	{
		...
		{
			FullscreenDialogNavigator.Open(
				new CertificationWindow(detailPanel.ResidentId, residentName),
				"Issue Certificate",
				residentName,
				"ResidentWorkspace",
				IconChar.FileContract,
		...
		{
			FullscreenDialogNavigator.Open(
				new PaymentWindow(detailPanel.ResidentId, residentName),
				"Record Payment",
				residentName,
				"ResidentWorkspace",
				IconChar.MoneyBill,
