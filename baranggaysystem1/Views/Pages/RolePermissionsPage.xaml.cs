...
public partial class RolePermissionsPage : UserControl, IRefreshable
{
	...
	private async void BtnDelete_Click(object sender, RoutedEventArgs e)
	{
		...
		if (selectedRole == null)
		{
			DialogService.Instance.ShowWarning("Please select a role to archive.", "Roles & Permissions");
		}
		else if (selectedRole.IsCoreRole)
		{
			DialogService.Instance.ShowWarning("Core roles cannot be archived.", "Roles & Permissions");
		}
		...
		else
		{
			var prompt = new ReasonPromptWindow(
				"Archive Role",
				"Explain why the role '" + selectedRole.Name + "' is being archived.",
				"Archive role")
			{
			...
			try
			{
				await _service.ArchiveRoleAsync(selectedRole.RoleId, prompt.Reason);
				await LoadAsync();
			}
			catch (Exception ex)
			{
				AppLogger.LogError("RolePermissionsPage archive failed.", ex);
				DialogService.Instance.ShowError(ex.Message, "Roles & Permissions");
			}
		}
	}
