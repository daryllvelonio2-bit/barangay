...
public partial class EmergencyContactsPage : UserControl, IRefreshable
{
    ...
    private async Task LoadAsync()
    {
        try
        {
            ...
            bool includeInactive = chkIncludeInactive.IsChecked == true;
            var table = await _service.LoadAsync(search, cat, priority, includeInactive);
            ApplyToGrid(table);
        }
        catch (Exception ex)
        {
    ...
    private void MainGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (mainGrid.SelectedItem is DataRowView row)
        {
            ...
            _selectedIsActive = row["is_active"] != DBNull.Value && Convert.ToInt32(row["is_active"]) == 1;
            selectedLabel.Text = row["agency_name"]?.ToString() ?? "";
            btnActiveState.Content = _selectedIsActive ? "Deactivate" : "Reactivate";
    ...
    private async void BtnActiveState_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            reason = DialogService.Instance.PromptForReason(
                "Deactivate Emergency Contact",
                "Why should this contact be removed from the active emergency directory?",
                "Deactivate");
            if (reason == null) return;
        }

        string action = _selectedIsActive ? "deactivate" : "reactivate";
        if (!DialogService.Instance.Confirm(
                $"{char.ToUpperInvariant(action[0]) + action.Substring(1)} \"{selectedLabel.Text}\"?",
                _selectedIsActive ? "Deactivate Contact" : "Reactivate Contact"))
            return;
        try
        {
            await _service.SetActiveAsync(_selectedId.Value, !_selectedIsActive, reason);
            await LoadAsync();
        }
        catch (Exception ex)
        {
            AppLogger.LogError("Update emergency contact state failed.", ex);
            DialogService.Instance.ShowError(ex.Message, "Unable to Update Contact");
        }
    }
