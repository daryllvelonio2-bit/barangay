...
public partial class TanodPatrolPage : UserControl, IRefreshable
{
    ...
    private void MainGrid_DoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        else if (_currentTab == TabMode.Members && _selectedId.HasValue)
            OpenMemberEditor(_selectedId.Value);
    }
    ...
    private void MainGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (mainGrid.SelectedItem is DataRowView row)
        {
            ...
            btnEditMember.Visibility = _currentTab == TabMode.Members ? Visibility.Visible : Visibility.Collapsed;
        }
        else
        {
            _selectedId = null;
    ...
    {
        if (_currentTab == TabMode.Members && _selectedId.HasValue)
            OpenMemberEditor(_selectedId.Value);
    }

    private void OpenMemberEditor(int tanodId)
    {
        var dlg = new TanodMemberWindow(tanodId);
        var adapter = new DialogContentAdapter(dlg);
        ...
        NavigationService.Instance.NavigateToFullscreen(new FullscreenViewConfig
        {
            ...
            Subtitle = "Update roster details or active status",
            OriginRoute = "TanodPatrol",
            Content = adapter,
