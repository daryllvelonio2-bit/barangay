...
public partial class MeetingsPage : UserControl, IRefreshable
{
    ...
    private void InitializeFilters()
    {
        ...
        btnLifecycle.Content = _isMeetingsTab ? "Cancel Meeting" : "Archive Document";
    }
    ...
    private void MainGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (mainGrid.SelectedItem is DataRowView row)
        {
            ...
            _selectedStatus = row["status"]?.ToString() ?? string.Empty;
            btnLifecycle.Content = _isMeetingsTab ? "Cancel Meeting" : "Archive Document";
            btnLifecycle.IsEnabled = _isMeetingsTab
                ? !string.Equals(_selectedStatus, "CANCELLED", StringComparison.OrdinalIgnoreCase)
                  && !string.Equals(_selectedStatus, "COMPLETED", StringComparison.OrdinalIgnoreCase)
                : !string.Equals(_selectedStatus, "ARCHIVED", StringComparison.OrdinalIgnoreCase);
    ...
    private void BtnEdit_Click(object sender, RoutedEventArgs e)
    {
        ...
        if ((_isMeetingsTab && string.Equals(_selectedStatus, "CANCELLED", StringComparison.OrdinalIgnoreCase))
            || (!_isMeetingsTab && string.Equals(_selectedStatus, "ARCHIVED", StringComparison.OrdinalIgnoreCase)))
        {
            DialogService.Instance.ShowWarning(
                _isMeetingsTab
                    ? "Cancelled meetings are locked. Create a new meeting if it must be rescheduled."
                    : "Archived documents are read-only. Create a new document or revision instead.",
                "Record Locked");
            return;
        }
        OpenEditor(_selectedId.Value);
    }
    ...
    private async void BtnLifecycle_Click(object sender, RoutedEventArgs e)
    {
        if (!_selectedId.HasValue) return;

        string action = _isMeetingsTab ? "cancel" : "archive";
        string recordLabel = _isMeetingsTab ? "meeting" : "document";
        string? reason = DialogService.Instance.PromptForReason(
            _isMeetingsTab ? "Cancel Meeting" : "Archive Document",
            $"Why do you want to {action} this {recordLabel}? This reason will be recorded in the audit trail.",
            _isMeetingsTab ? "Cancel Meeting" : "Archive");
        if (reason == null) return;

        if (!DialogService.Instance.Confirm(
                $"Confirm that you want to {action} \"{selectedLabel.Text}\".\n\nThe record will be retained for reporting and audit.",
                _isMeetingsTab ? "Confirm Cancellation" : "Confirm Archive"))
            return;

        try
        {
            if (_isMeetingsTab)
                await _service.CancelMeetingAsync(_selectedId.Value, reason);
            else
                await _service.ArchiveResolutionAsync(_selectedId.Value, reason);
            await LoadAsync();
        }
        catch (Exception ex)
        {
            AppLogger.LogError("Record lifecycle update failed.", ex);
            DialogService.Instance.ShowError($"Unable to {action} this {recordLabel}: {ex.Message}");
        }
    }
