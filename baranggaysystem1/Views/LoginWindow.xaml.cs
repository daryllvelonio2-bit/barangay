...
public partial class LoginWindow : Window
{
	...
	private bool _isHandlingCustomProfile;

	private bool _isConnectionProfileDropDownOpen;

	private bool _suppressCustomProfilePopupOnClose;

	private string _lastSelectedConnectionProfileKey = "sqlite";
	...
	private void InitializeConnectionOptions()
	{
		...
		try
		{
			...
			_lastSelectedConnectionProfileKey = DbConnectionSettingsStore.LoadSelectedProfileKeyOrDefault();
	...
	private static ConnectionStatusSnapshot EvaluateConnectionStatus(DatabaseConnectionOption option, bool persistSelection)
	{
		...
		if (option.UsesSqlite)
		{
			if (!OfflineDatabaseSupport.EnsureInitialised())
			{
				...
				return BuildDisconnectedSnapshot("The writable SQLite database could not be prepared. Check the application log for the exact file or schema error.");
			}
			DBConnection.SetRuntimeSqliteSelection(isSelected: true);
	...
	{
		if (_isLoadingConnectionOptions || _isHandlingCustomProfile)
		{
			return;
		}
		DatabaseConnectionOption selectedConnectionOption = GetSelectedConnectionOption();
		...
		if (DbConnectionSettingsStore.IsCustomProfileKey(selectedConnectionOption.Key))
		{
			_suppressCustomProfilePopupOnClose = true;
			await HandleCustomProfileSelectionAsync();
			return;
		}
		_suppressCustomProfilePopupOnClose = false;
		_lastSelectedConnectionProfileKey = selectedConnectionOption.Key;
		await RefreshConnectionStatusAsync(selectedConnectionOption, persistSelection: true, blockLogin: true);
	}

	private void CmbConnectionProfile_DropDownOpened(object sender, EventArgs e)
	{
		_isConnectionProfileDropDownOpen = true;
	}

	private async void CmbConnectionProfile_DropDownClosed(object sender, EventArgs e)
	{
		if (!_isConnectionProfileDropDownOpen)
		{
			return;
		}

		_isConnectionProfileDropDownOpen = false;
		if (_suppressCustomProfilePopupOnClose)
		{
			_suppressCustomProfilePopupOnClose = false;
			return;
		}
		...
		DatabaseConnectionOption selectedConnectionOption = GetSelectedConnectionOption();
		if (selectedConnectionOption == null || !DbConnectionSettingsStore.IsCustomProfileKey(selectedConnectionOption.Key) || _isLoadingConnectionOptions || _isHandlingCustomProfile || _isConnectionRefreshRunning || _isLoginRunning)
		{
			return;
		}
	...
	private async Task HandleCustomProfileSelectionAsync()
	{
		string previousKey = _lastSelectedConnectionProfileKey;
		DatabaseConnectionOption customOption = DbConnectionSettingsStore.GetOptionOrDefault("custom");
		CustomDatabaseConnectionWindow customDatabaseConnectionWindow = new CustomDatabaseConnectionWindow(customOption.Profile);
		if (DialogService.Instance.ShowDialog(customDatabaseConnectionWindow) != true)
		{
			RestoreConnectionProfileSelection(previousKey);
			return;
		}
		DatabaseConnectionProfile connectionProfile = customDatabaseConnectionWindow.ConnectionProfile;
		string backupZipPath = customDatabaseConnectionWindow.BackupZipPath ?? string.Empty;
		try
		{
			_isHandlingCustomProfile = true;
			DbConnectionSettingsStore.SaveSelectedProfile("custom", connectionProfile);
			DBConnection.SetRuntimeSqliteSelection(isSelected: false);
			DBConnection.SetRuntimeConnectionString(DbConnectionSettingsStore.BuildConnectionString(connectionProfile));
			if (!string.IsNullOrWhiteSpace(backupZipPath))
			{
				string text = Path.GetFullPath(backupZipPath);
				await Task.Run(() =>
				{
					if (!DatabaseAutoCreator.EnsureDatabaseShellExists(connectionProfile))
					{
						throw new InvalidOperationException("The selected database could not be created on the server.");
					}
					DBConnection.SetRuntimeConnectionString(DbConnectionSettingsStore.BuildConnectionString(connectionProfile));
					BackupService.RestoreBackupFile(text);
				});
			}
			InitializeConnectionOptions();
			_lastSelectedConnectionProfileKey = "custom";
			DatabaseConnectionOption selectedConnectionOption = GetSelectedConnectionOption();
			if (selectedConnectionOption != null)
			{
				await RefreshConnectionStatusAsync(selectedConnectionOption, persistSelection: false, blockLogin: true);
			}
		}
		catch (Exception ex)
		{
			RestoreConnectionProfileSelection(previousKey);
			DialogService.Instance.ShowError("Failed to save the custom database profile: " + ex.Message, "Custom Database Setup");
		}
		finally
		{
			_isHandlingCustomProfile = false;
		}
	}

	private void RestoreConnectionProfileSelection(string profileKey)
	{
		_isLoadingConnectionOptions = true;
		try
		{
			cmbConnectionProfile.SelectedValue = profileKey;
		}
		finally
		{
			_isLoadingConnectionOptions = false;
	...
	{
		// Show welcome setup on first launch
		if (Views.Dialogs.WelcomeSetupWindow.ShouldShow())
		{
			var welcome = new Views.Dialogs.WelcomeSetupWindow();
			welcome.ShowDialog();
		}
