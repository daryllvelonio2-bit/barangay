...
public partial class CertificateFormPanel : FullscreenFormBase
{
    ...
    private readonly bool _businessOnly;
    ...
    public CertificateFormPanel(CertificateDialogMode mode)
        : this(mode, 0, string.Empty, null, false)
    {
    }

    public CertificateFormPanel(CertificateDialogMode mode, bool businessOnly)
        : this(mode, 0, string.Empty, null, businessOnly)
    {
    }
    ...
    public CertificateFormPanel(CertificateDialogMode mode, int residentId, string residentName)
        : this(mode, residentId, residentName, null, false)
    {
    }
    ...
    public CertificateFormPanel(CertificateDialogMode mode, int requestId, bool loadExisting)
        : this(mode, 0, string.Empty, requestId, false)
    {
    }

    private CertificateFormPanel(CertificateDialogMode mode, int residentId, string residentName, int? requestId, bool businessOnly)
    {
        InitializeComponent();
    ...
    private async Task LoadDataAsync()
    {
        ...
        try
        {
            // Load document types (independently - don't let failure block residents)
            try
            {
                var types = await _certificateService.GetDocumentTypesAsync();
                DocumentTypes.Clear();
                foreach (var type in types.Where(type => !_businessOnly ||
                    type.Code.Contains("BUS", StringComparison.OrdinalIgnoreCase) ||
                    type.Code.Contains("PERMIT", StringComparison.OrdinalIgnoreCase) ||
                    type.Name.Contains("BUSINESS", StringComparison.OrdinalIgnoreCase) ||
                    type.Name.Contains("PERMIT", StringComparison.OrdinalIgnoreCase)))
                {
            ...
            {
                AppLogger.LogWarning("Failed to load document types for certificate form.", ex);
            }

            // Load resident options if picker is visible (independently)
            if (residentPickerSection.Visibility == Visibility.Visible)
            {
                ...
                {
                    AppLogger.LogWarning("Failed to load residents for certificate form.", ex);
                }
            }
            ...
            if (_requestId.HasValue && _requestId.Value > 0)
            {
                ...
                {
                    AppLogger.LogWarning("Failed to load existing request.", ex);
                }
            }
            ...
            {
                cmbDocumentType.SelectedItem = (_businessOnly
                    ? DocumentTypes.FirstOrDefault()
                    : DocumentTypes.FirstOrDefault(t =>
