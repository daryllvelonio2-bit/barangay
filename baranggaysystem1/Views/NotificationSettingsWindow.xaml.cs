...
{
    private string _storedSmtpPassword = string.Empty;
    private string _storedSmsToken = string.Empty;
    ...
    private void LoadCurrentSettings()
    {
        try
        {
            ...
            _storedSmtpPassword = smtp.Password ?? string.Empty;
            smtpPassBox.Password = string.Empty;
            ...
            _storedSmsToken = sms.ApiToken ?? string.Empty;
            smsTokenBox.Password = string.Empty;
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(smtp.Password))
                _storedSmtpPassword = smtp.Password;
            ...
            if (!string.IsNullOrWhiteSpace(sms.ApiToken))
                _storedSmsToken = sms.ApiToken;
    ...
    private void TestSmtp_Click(object sender, RoutedEventArgs e)
    {
        ...
        var smtp = new SmtpSettings
        {
            ...
            FromName = smtpFromNameBox.Text?.Trim() ?? "Barangay System",
            Username = smtpUserBox.Text?.Trim() ?? "",
            Password = string.IsNullOrWhiteSpace(smtpPassBox.Password)
                ? _storedSmtpPassword
    ...
    private void TestSms_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            ApiUrl = smsApiUrlBox.Text?.Trim() ?? "",
            ApiToken = string.IsNullOrWhiteSpace(smsTokenBox.Password)
                ? _storedSmsToken
                : smsTokenBox.Password,
            SenderName = smsSenderBox.Text?.Trim() ?? "Barangay"
        };
