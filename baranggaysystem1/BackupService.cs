...
internal static class BackupService
{
	...
	internal static string RestoreBackupFile(string backupPath)
	{
		...
		try
		{
			...
			string inputSqlPath = PrepareRestoreSql(fullPath, ((MySqlBaseConnectionStringBuilder)resolvedConnectionStringBuilder).Database, out tempSqlPath);
	...
	private static string PrepareRestoreSql(string backupPath, string? targetDatabaseName, out string? tempSqlPath)
	{
		tempSqlPath = null;
		...
		{
			if (string.IsNullOrWhiteSpace(normalizedTargetDatabaseName))
			{
				return backupPath;
			}
			tempSqlPath = Path.Combine(Path.GetTempPath(), $"bms-restore-{Guid.NewGuid():N}.sql");
			File.Copy(backupPath, tempSqlPath, overwrite: true);
			RewriteRestoreSqlForTargetDatabase(tempSqlPath, normalizedTargetDatabaseName);
			return tempSqlPath;
		}
	...
	private static void RewriteRestoreSqlForTargetDatabase(string sqlPath, string targetDatabaseName)
	{
		if (string.IsNullOrWhiteSpace(sqlPath) || !File.Exists(sqlPath) || string.IsNullOrWhiteSpace(targetDatabaseName))
		{
			return;
		}

		string safeDatabaseName = targetDatabaseName.Trim().Replace("`", "``", StringComparison.Ordinal);
		string[] lines = File.ReadAllLines(sqlPath);
		List<string> rewritten = new List<string>(lines.Length + 1);
		bool foundUseStatement = false;

		foreach (string line in lines)
		{
			string trimmed = line.TrimStart();
			if (Regex.IsMatch(trimmed, "^(?i:CREATE)\\s+(?i:DATABASE)\\b") || Regex.IsMatch(trimmed, "^(?i:DROP)\\s+(?i:DATABASE)\\b"))
			{
				continue;
			}

			if (Regex.IsMatch(trimmed, "^(?i:USE)\\s+.+;\\s*$"))
			{
				rewritten.Add($"USE `{safeDatabaseName}`;");
		...
		{
			rewritten.Insert(0, $"USE `{safeDatabaseName}`;");
		}

		File.WriteAllLines(sqlPath, rewritten);
	}

	private static string NormalizeDatabaseName(string? databaseName)
	{
		string text = (databaseName ?? string.Empty).Trim();
		if (string.IsNullOrWhiteSpace(text))
		{
			return string.Empty;
		}
	...
	private static List<string> BuildConnectionArgs(MySqlConnectionStringBuilder csb, string? defaultsFilePath)
	{
		...
		// Note: do NOT add --database here. mysqldump does not support that option.
		// The target database is passed via --databases in RunMySqlDump.
